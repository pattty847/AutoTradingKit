...
from atklip.controls import PD_MAType,IndicatorType,MOM
from atklip.controls.models import MOMModel
...
class BasicMOM(PlotDataItem):
    ...
    def __init__(self,get_last_pos_worker, chart,panel,clickable=True) -> None:
        ...
        self.INDICATOR  = MOM(self.has["inputs"]["source"], self.model.__dict__)
    ...
    @property
    def model(self) -> dict:
        return MOMModel(self.id,"MOM",self.chart.jp_candle.source_name,
    ...
    def regen_indicator(self,setdata):
        ...
        self.has["name"] = f"MOM {self.has["inputs"]["length"]} {self.has["inputs"]["type"]}"
    ...
    def update_data(self,setdata):
        ...
        setdata.emit((xdata,y_data))
        self.last_pos.emit((IndicatorType.MOM,y_data[-1]))
