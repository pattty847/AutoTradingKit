...
def trix(close, length=None, signal=None,mamode="ema", scalar=None, drift=None, offset=None, **kwargs):
    """Indicator: Trix (TRIX)"""
    # Validate Arguments
    ...
    # Calculate Result
    ema1 = ma(name=mamode,source=close, length=length, **kwargs)
    ema2 = ma(name=mamode,source=ema1, length=length, **kwargs)
    ema3 = ma(name=mamode,source=ema2, length=length, **kwargs)
