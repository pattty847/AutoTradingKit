...
from typing import List, Any, Callable, Union, Type,Dict
...
from atklip.indicators.talipp.input import SamplingPeriodType, Sampler
...
class Indicator(Sequence):
    ...
    def __init__(self,
        ...
        self._dict_time_value: Dict[int,Any] = {}
        self._type = _type
        self.input_indicator = None
        self.output_times: ListAny = []
    ...
    def re_initialize(self,input_values):
        self.remove_all()
        if input_values is not None:
            for value in input_values:
                self.add(value)
        if self.input_indicator is not None:
            for value in self.input_indicator:
                self.add(value)
            self.input_indicator.add_output_listener(self)
    ...
    def caculate_new_value(self,input_value:OHLCV):
        _time = None    
        if input_value is not None and self.input_modifier is not None:
            input_value = self.input_modifier(input_value)
        elif self._type in ["close", "low", "high", "volume", "open","hl2","hlc3","ohlc4"]:
    ...
    def add(self, value: Any|OHLCV|List[OHLCV]) -> None:
        """Add a new input value.
        ...
        if (self.input_sampler is not None
            ...
            [self.caculate_new_value(input_value) for input_value in value]
    ...
    def _purge_oldest_output_value(self, size: int) -> None:
        if len(self.output_values) > size:
            self.output_values = self.output_values[size:]
            if len(self.output_values) == len(self.output_times):
                self.output_times = self.output_times[size:]
