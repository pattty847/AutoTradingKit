...
class UO(Indicator):
    ...
    def _calculate_new_value(self) -> Any:
        ...
        if not has_valid_values(self.buy_press, self.slow_period):
            return None
        _fast_period = float(sum(self.true_range[-self.fast_period:]))
        _mid_period = float(sum(self.true_range[-self.mid_period:]))
        _slow_period = float(sum(self.true_range[-self.slow_period:]))
        if _fast_period == 0 or _mid_period == 0\
            or _slow_period == 0:
                return None
        avg_fast = sum(self.buy_press[-self.fast_period:]) / _fast_period
        avg_mid = sum(self.buy_press[-self.mid_period:]) / _mid_period
        avg_slow = sum(self.buy_press[-self.slow_period:]) / _slow_period
