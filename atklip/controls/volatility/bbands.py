...
from atklip.controls.pandas_ta.volatility import bbands
import numpy as np
import pandas as pd
...
class BBANDS(QObject):
    ...
    def __init__(self, _candles, dict_ta_params) -> None:
        ...
        self._name = (
            f"{self.mamode.lower()} {self.source} {self.length} {self.std_dev_mult}"
        )
    ...
    def callback_update(self, future: Future):
        ...
        last_cb = df["cb"].iloc[-1]
        last_ub = df["ub"].iloc[-1]
        self.df.loc[self.df.index[-1], ["index", "lb", "cb", "ub"]] = [
            last_index,
            last_lb,
            last_cb,
            last_ub,
