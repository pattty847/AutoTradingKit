...
class KC(QObject):
    ...
    def first_gen_data(self):
        ...
        process = HeavyProcess(
            self.calculate, self.callback_first_gen, df, self.length, self.offset
        )
        process.start()
    ...
    def add(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            ...
            process = HeavyProcess(
                self.calculate, self.callback_add, df, self.length, self.offset
            )
            process.start()
    ...
    def update(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            df: pd.DataFrame = self._candles.get_df(self.length + 5)
            process = HeavyProcess(
                self.calculate, self.callback_update, df, self.length, self.offset
            )
            process.start()
        else:
            pass
    ...
    def callback_add(self, future: Future):
        ...
        new_frame = pd.DataFrame(
            {"index": [last_index], "lb": [last_lb], "cb": [last_cb], "ub": [last_ub]}
        )
    ...
    def callback_update(self, future: Future):
        ...
        last_cb = df["cb"].iloc[-1]
        last_ub = df["ub"].iloc[-1]
        self.df.loc[self.df.index[-1]] = [last_index, last_lb, last_cb, last_ub]
        self.xdata[-1], self.lb[-1], self.cb[-1], self.ub[-1] = (
            last_index,
            last_lb,
            last_cb,
            last_ub,
