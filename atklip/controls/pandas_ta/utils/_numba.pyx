...
def nb_pvi(np_close, np_volume, initial):
    @njit(cache=True)
    def _nb_pvi(np_close, np_volume, initial):
        ...
        return result
    return _nb_pvi(np_close, np_volume, initial)



def nb_atrts(x, ma, atr_, length, ma_length):
    @njit(cache=True)
    def _nb_atrts(x, ma, atr_, length, ma_length):
        m = x.size
    ...
    return _nb_atrts(x, ma, atr_, length, ma_length)



def nb_rolling_hl(np_high, np_low, window_size):
    @njit(cache=True)
    def _nb_rolling_hl(np_high, np_low, window_size):
        m = np_high.size
        idx = zeros(m)
    ...
    return _nb_rolling_hl(np_high, np_low, window_size)

def nb_find_zigzags(idx, swing, value, deviation):
    @njit(cache=True)
    def _nb_find_zigzags(idx, swing, value, deviation):
    ...
    return _nb_find_zigzags(idx, swing, value, deviation) 

def nb_map_zigzag(idx, swing, value, deviation, n):
    @njit(cache=True)
    def _nb_map_zigzag(idx, swing, value, deviation, n):
        swing_map = zeros(n)
        value_map = zeros(n)
        dev_map = zeros(n)
    ...
    return _nb_map_zigzag(idx, swing, value, deviation, n)
...
def nb_trendflex(x, n, k, alpha, pi, sqrt2):
    @njit(cache=True)
    def _nb_trendflex(x, n, k, alpha, pi, sqrt2):
        ...
        return result
    return _nb_trendflex(x, n, k, alpha, pi, sqrt2)

def nb_ht_trendline(x):
    @njit(cache=True)
    def _nb_ht_trendline(x):
...
def nb_exponential_decay(x, n):
    ...
    def _nb_exponential_decay(x, n):
        ...
        return result
    return _nb_exponential_decay(x, n)

def nb_alpha(low_atr, high_atr, momo_threshold):
    """
    Calculate the alpha value based on low_atr, high_atr, and momo_threshold.
    The function uses Numba's njit for optimization.
    """
    @njit(cache=True)
    def _nb_alpha(low_atr, high_atr, momo_threshold):
        ...
        return result
    return _nb_alpha(low_atr, high_atr, momo_threshold)

def nb_exhc(x, n, cap, lb, ub, show_all):
    @njit(cache=True)
    def _nb_exhc(x, n, cap, lb, ub, show_all):
        x_diff = nb_idiff(x, n)
...
def np_reflex(x, n, k, alpha, pi, sqrt2):
    ...
    def _np_reflex(x, n, k, alpha, pi, sqrt2):
        ...
        return result
    return _np_reflex(x, n, k, alpha, pi, sqrt2)

def np_cdl_inside(high, low):
    @njit(cache=True)
    def _np_cdl_inside(high, low):
        hdiff = where(high - roll(high, 1) < 0, 1, 0)
        ldiff = where(low - roll(low, 1) > 0, 1, 0)
        return hdiff & ldiff
    
    return _np_cdl_inside(high, low)
def np_ha(np_open, np_high, np_low, np_close):
    ...
    return _np_ha(np_open, np_high, np_low, np_close)
def nb_roc(x, n, k):
    @njit(cache=True)
    def _nb_roc(x, n, k):
        return k * nb_idiff(x, n) / nb_shift(x, n)
    return _nb_roc(x, n, k)
...
def fibonacci(n, weighted):
    ...
    def _fibonacci(n, weighted):
        ...
        return result
    return _fibonacci(n, weighted)

def nb_non_zero_range(x, y):
    @njit(cache=True)
    def _nb_non_zero_range(x, y):
        diff = x - y
        if diff.any() == 0:
            diff += finfo(float64).eps
        return diff
    return _nb_non_zero_range(x, y)

def nb_wma(x, n, asc, prenan):
    @njit(cache=True)
    def _nb_wma(x, n, asc, prenan):
        m = x.size
...
def nb_ssf_everget(x, n, pi, sqrt2):
    @njit(cache=True)
    def _nb_ssf_everget(x, n, pi, sqrt2):
        ...
        return result
    return _nb_ssf_everget(x, n, pi, sqrt2)
...
def pivot_camarilla(high, low, close):
    @njit(cache=True)
    def _pivot_camarilla(high, low, close):
        tp = (high + low + close) / 3
    ...
    return _pivot_camarilla(high, low, close)

def pivot_classic(high, low, close):
    @njit(cache=True)
    def _pivot_classic(high, low, close):
        tp = (high + low + close) / 3
        hl_range = nb_non_zero_range(high, low)
        ...
        return tp, s1, s2, s3, s4, r1, r2, r3, r4
    return _pivot_classic(high, low, close)

def pivot_demark(open_, high, low, close):
    @njit(cache=True)
    def _pivot_demark(open_, high, low, close):
        if (open_ == close).all():
        ...
        return tp, s1, r1
    return _pivot_demark(open_, high, low, close)

def pivot_fibonacci(high, low, close):
    @njit(cache=True)
    def _pivot_fibonacci(high, low, close):
        tp = (high + low + close) / 3
        hl_range = nb_non_zero_range(high, low)
    ...
    return _pivot_fibonacci(high, low, close)

def pivot_traditional(high, low, close):
    @njit(cache=True)
    def _pivot_traditional(high, low, close):
        tp = (high + low + close) / 3
        hl_range = nb_non_zero_range(high, low)
        ...
        return tp, s1, s2, s3, s4, r1, r2, r3, r4
    return _pivot_traditional(high, low, close)

def pivot_woodie(open_, high, low):
    @njit(cache=True)
    def _pivot_woodie(open_, high, low):
        ...
        return tp, s1, s2, s3, s4, r1, r2, r3, r4
    return _pivot_woodie(open_, high, low)
...
def nb_mama(x, fastlimit, slowlimit, prenan):
    @njit(cache=True)
    def _nb_mama(x, fastlimit, slowlimit, prenan):
        a, b, m = 0.0962, 0.5769, x.size
        ...
        return mama, fama
    return _nb_mama(x, fastlimit, slowlimit, prenan)
