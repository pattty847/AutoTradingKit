...
def calculate_pl_with_fees(
    entry_price: float,
    exit_price: float,
    capital=1000,
    proportion_closed=100,
    leverage: int = 1,
    taker_fee: float = 0,
    maker_fee: float = 0,
...
def calculate_recommended_capital(
    entry_price: float,
    ...
    effective_loss_per_unit = leveraged_loss_per_unit + (
        total_taker_fee / (entry_price * leverage)
    )
    recommended_capital = capital_to_risk / effective_loss_per_unit
    return round(recommended_capital, 4)


def calculate_recommended_capital_base_on_loss_capital(
    entry_price: float,
    stop_loss_price: float,
...
total_capital = st.number_input(
    "Total Capital - _The total amount of capital available for trading._",
    value=10000.0,
    format="%.2f",
)
capital = st.number_input(
    "Capital - _The amount of money you are investing in this position._",
    value=1000.0,
    format="%.2f",
)
leverage = st.number_input("Leverage", value=10)
risk_percentage = st.number_input(
    "Risk Percentage - _The percentage of your total trading capital that you are willing to risk on this trade._",
    value=5.0,
    format="%.1f",
)
...
with col1:
    exit_price_tp1 = st.number_input(
        "Exit Price TP1 ($)", value=61000.00, format="%.2f"
    )
    proportion_closed_tp1 = st.number_input("Proportion Closed TP1 (%)", value=0.5)

with col2:
    exit_price_tp2 = st.number_input(
        "Exit Price TP2 ($)", value=62000.00, format="%.2f"
    )
    proportion_closed_tp2 = st.number_input("Proportion Closed TP2 (%)", value=0.3)

with col3:
    exit_price_tp3 = st.number_input(
        "Exit Price TP3 ($)", value=63000.00, format="%.2f"
    )
...
if st.button("Calculate"):
    ...
    if is_long:
        if stop_loss_price >= entry_price:
            st.error(
                "For LONG positions, the stop loss price must be lower than the entry price."
            )
            valid_input = False
        if (
            exit_price_tp1 <= entry_price
            or exit_price_tp2 <= entry_price
            or exit_price_tp3 <= entry_price
        ):
            st.error(
                "For LONG positions, the take profit prices must be higher than the entry price."
            )
            valid_input = False
    else:
        if stop_loss_price <= entry_price:
            st.error(
                "For SHORT positions, the stop loss price must be higher than the entry price."
            )
            valid_input = False
        if (
            exit_price_tp1 >= entry_price
            ...
            st.error(
                "For SHORT positions, the take profit prices must be lower than the entry price."
            )
            valid_input = False
    ...
    if valid_input:
        ...
        if proportion_closed_tp1 > 0:
            pl_tp1 = calculate_pl_with_fees(
                entry_price,
                exit_price_tp1,
                capital,
                leverage,
                proportion_closed_tp1,
                taker_fee,
                maker_fee,
            )
            total_pl += pl_tp1
        ...
        if proportion_closed_tp2 > 0 and total_proportion < 1.0:
            pl_tp2 = calculate_pl_with_fees(
                entry_price,
                exit_price_tp2,
                capital,
                leverage,
                proportion_closed_tp2,
                taker_fee,
                maker_fee,
                is_stop_loss=False,
            )
        ...
        if proportion_closed_tp3 > 0 and total_proportion < 1.0:
            pl_tp3 = calculate_pl_with_fees(
                entry_price,
                exit_price_tp3,
                capital,
                leverage,
                proportion_closed_tp3,
                taker_fee,
                maker_fee,
                is_stop_loss=False,
            )
        ...
        if total_proportion < 1.0:
            pl_remaining = calculate_pl_with_fees(
                entry_price,
                entry_price,
                capital,
                leverage,
                1.0 - total_proportion,
                taker_fee,
                maker_fee,
                is_stop_loss=False,
            )
        ...
        pl_stop_loss = calculate_pl_with_fees(
            entry_price,
            stop_loss_price,
            capital,
            leverage,
        ...
        recommended_capital = calculate_recommended_capital(
            entry_price,
            stop_loss_price,
            total_capital,
            risk_percentage,
        ...
        recommended_capital_loss_capital = (
            calculate_recommended_capital_base_on_loss_capital(
                entry_price, stop_loss_price, 50, leverage, taker_fee
            )
        )
        ...
        st.markdown(
            f"**Recommended capital to risk {risk_percentage}% of total capital:** :green[${recommended_capital}]"
        )
        st.markdown(
            f"**Recommended capital base on loss_capital:** :green[${recommended_capital_loss_capital}]"
        )
        ...
        if total_proportion < 1.0:
            st.markdown(
                f"**P/L for remaining position (if SL is hit at entry price):** :green[${pl_remaining}]"
            )
        st.markdown(f"**P/L if Stop Loss is hit directly:** :red[${pl_stop_loss}]")

        # Check that the total proportion is 100%
        if (
            proportion_closed_tp1 + proportion_closed_tp2 + proportion_closed_tp3
        ) != 1.0:
            st.warning(
                "Warning: The total proportion of closed positions does not equal 100%. Please check the proportions."
            )
