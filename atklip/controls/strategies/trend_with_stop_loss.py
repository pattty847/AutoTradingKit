...
def trend_with_stoploss(
    ...
    # Tính toán MACD
    macd_line, histogram, signalma = calculate_macd(
        data,
    ...
    data["short_stoploss_diff"] = data["short_stoploss"] < data["short_stoploss"].shift(
        1
    )
    ...
    data["Uptrend"] = (
        (macd_line > 0)
        & (data["long_stoploss_diff"])
        & (
            (data["macd_line_diff"])
            | (data["histogram_diff"] == False)
            | (data["signalma_diff"])
        )
    )
    ...
    data["Downtrend"] = (
        (macd_line < 0)
        & (data["short_stoploss_diff"])
        & (
            ((data["macd_line_diff"] == False))
            | (data["histogram_diff"])
            | (data["signalma_diff"] == False)
        )
    )
...
class TrendWithStopLoss(QObject):
    ...
    def get_data(self, start: int = 0, stop: int = 0):
        ...
        if start == 0 and stop == 0:
            x_data = self.xdata
            long_stoploss, short_stoploss, Uptrend, Downtrend = (
                self.long_stoploss,
                self.short_stoploss,
                self.Uptrend,
                self.Downtrend,
            ...
            long_stoploss, short_stoploss, Uptrend, Downtrend = (
                ...
                self.Uptrend[start:stop],
                self.Downtrend[start:stop],
            )
    ...
    def first_gen_data(self):
        ...
        _len = min(
            [len(long_stoploss), len(short_stoploss), len(Uptrend), len(Downtrend)]
        )
    ...
    def add(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            ...
            self.long_stoploss = np.concatenate(
                (self.long_stoploss, np.array([long_stoploss.iloc[-1]]))
            )
            self.short_stoploss = np.concatenate(
                (self.short_stoploss, np.array([short_stoploss.iloc[-1]]))
            )
            self.Uptrend = np.concatenate((self.Uptrend, np.array([Uptrend.iloc[-1]])))
            self.Downtrend = np.concatenate(
                (self.Downtrend, np.array([Downtrend.iloc[-1]]))
            )
    ...
    def update(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            ...
            self.df.iloc[-1] = [
                new_candle.index,
                long_stoploss.iloc[-1],
                short_stoploss.iloc[-1],
                Uptrend.iloc[-1],
                Downtrend.iloc[-1],
            ]
            (
