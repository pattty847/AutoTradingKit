...
class SMOOTH_CANDLE(QObject):
    ...
    def __init__(
        self, chart, _candles, mamode, ma_leng, dict_candle_params: dict = {}
    ) -> None:
    ...
    def change_input(self, candles=None, dict_candle_params: dict = {}):
        ...
        if dict_candle_params != {}:
            ...
            self.chart_id = dict_candle_params["chart_id"]
            self.name: str = dict_candle_params.get("name")
            source_name = f"{self.chart_id}-{self.canlde_id}-{self.id_exchange}-{self.source}-{self.name}-{self.symbol}-{self.interval}-{self.mamode.name}-{self.ma_leng}".encode(
                "utf-8"
            ).decode(
                "utf-8"
            )
    ...
    def get_ma_ohlc_at_index(self, df: pd.DataFrame | None, index):
        if df is None:
            df = self.df
        _open, _high, _low, _close, _hl2, _hlc3, _ohlc4, _volume, _time, _index = (
            df["open"].iloc[index],
            df["high"].iloc[index],
            df["low"].iloc[index],
            df["close"].iloc[index],
        ...
        if _open != None and _high != None and _low != None and _close != None:
            _open, _high, _low, _close, _hl2, _hlc3, _ohlc4 = (
                round(_open, self.precision),
                round(_high, self.precision),
                round(_low, self.precision),
                round(_close, self.precision),
                round(_hl2, self.precision),
                round(_hlc3, self.precision),
                round(_ohlc4, self.precision),
            )
            return (
                _open,
                _high,
    ...
    def compute(self, index, i: int):
        ...
        if ohlc != []:
            _open, _high, _low, _close, _hl2, _hlc3, _ohlc4, _volume, _time = (
                ohlc[0],
                ohlc[1],
                ohlc[2],
                ohlc[3],
    ...
    def callback_update_ma_ohlc(self, future: Future):
        ...
        _open, _high, _low, _close, hl2, hlc3, ohlc4, _volume, _time, _index = (
            self.get_ma_ohlc_at_index(None, -1)
        )
        ha_candle = OHLCV(
            _open, _high, _low, _close, hl2, hlc3, ohlc4, _volume, _time, _index
        )
