...
class HEIKINASHI(QObject):
    ...
    def compute(self, candle: OHLCV):
        if len(self.candles) == 0:
            ...
            ha_open = round(
                (self.candles[-1].open + self.candles[-1].close) / 2, self.precision
            )
            ha_close = round(
                (candle.open + candle.high + candle.low + candle.close) / 4,
                self.precision,
    ...
    def load_historic_data(self, n):
        ...
        [
            self.update_historic(new_candle, update_candles[i], i)
            for i in range(len(update_candles))
        ]
    ...
    def update_historic(self, new_candle: List[OHLCV], candle: OHLCV, i: int):
        if i == 0:
            ...
            ha_open = round(
                (self.candles[i - 1].open + self.candles[i - 1].close) / 2,
                self.precision,
            )
            ha_close = round(
                (candle.open + candle.high + candle.low + candle.close) / 4,
        ...
        if self.first_gen:
            ...
            if len(self.candles) < 2:
                ...
                if _is_add_candle == True:
                    # _index = new_candle.index
                    ha_open = round(
                        (self.candles[-1].open + self.candles[-1].close) / 2,
                        self.precision,
                    )
                    ha_close = round(
                        (
                    ...
                    hlc3 = round((ha_high + ha_low + ha_close) / 3, self.precision)
                    ohlc4 = round(
                        (ha_open + ha_high + ha_low + ha_close) / 4, self.precision
                    )
                    ...
                    new_row = pd.DataFrame(
                        [data.__dict__ for data in self.candles[-1:]]
                    )
                    ...
                    ha_open = round(
                        (self.candles[-2].open + self.candles[-2].close) / 2,
                        self.precision,
                    )
                    ha_close = round(
                        (
                    ...
                    if (
                        ha_candle.close != self.candles[-1].close
                        or ha_candle.high != self.candles[-1].high
                        or ha_candle.low != self.candles[-1].low
                        or ha_candle.open != self.candles[-1].open
                    ):
