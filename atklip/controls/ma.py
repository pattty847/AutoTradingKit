...
class MA(QObject):
    ...
    def callback_first_gen(self, future: Future):
        ...
        self.df = pd.DataFrame({"index": _index, "data": data})
    ...
    def _gen_data(df, mamode, source, length, zl_mode):
        data = (
            ma(mamode, source=df[source], length=length, mamode=zl_mode)
            .dropna()
            .round(6)
        )
        _len = len(data)
    ...
    def call_back_add_historic(self, future: Future):
        _index, data = future.result()
        _df = pd.DataFrame({"index": _index, "data": data})
    ...
    def callback_add(self, future: Future):
        ...
        new_frame = pd.DataFrame({"index": [_index], "data": [_data]})
