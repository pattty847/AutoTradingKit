...
class ExchangeManager:
    ...
    def add_exchange(self, id_exchange, chart_id, symbol, interval, apikey, secretkey):
        self.is_stop = False
        client = self.set_client_exchange(
            id_exchange, chart_id, symbol, interval, apikey, secretkey
        )
        ws = self.set_ws_exchange(
            id_exchange, chart_id, symbol, interval, apikey, secretkey
        )
    ...
    def set_ws_exchange(
        self, id_exchange, chart_id, symbol, interval, apikey, secretkey
    ):
        ...
        if chart == None:
            self.map_chart_exchange[f"ws-{chart_id}-{symbol}-{interval}"] = {
                f"ws-{id_exchange}": ws
            }
        else:
            self.map_chart_exchange[f"ws-{chart_id}-{symbol}-{interval}"].update(
                {f"ws-{id_exchange}": ws}
            )
        return ws

    def set_client_exchange(
        self, id_exchange, chart_id, symbol, interval, apikey, secretkey
    ):
        client = CryptoExchange().setupEchange(
            apikey=apikey, secretkey=secretkey, exchange_name=id_exchange
        )
        chart = self.map_chart_exchange.get(f"client-{chart_id}-{symbol}-{interval}")
        if chart == None:
            self.map_chart_exchange[f"client-{chart_id}-{symbol}-{interval}"] = {
                f"client-{id_exchange}": client
            }
        else:
            self.map_chart_exchange[f"client-{chart_id}-{symbol}-{interval}"].update(
                {f"client-{id_exchange}": client}
            )
        return client
    ...
    async def reload_markets(self, id_exchange, chart_id, symbol, interval):
        ...
        while True:
            try:
                ...
                n += 1
                client_socket = self.get_client_exchange(
                    id_exchange, chart_id, symbol, interval
                )
                ws_socket = self.get_ws_exchange(
                    id_exchange, chart_id, symbol, interval
                )
                ...
                if n == 60:
                    ...
                    print(
                        f"{id_exchange}-{chart_id}-{symbol}-{interval} --- Markets reloaded"
                    )
    ...
    async def remove_exchange(self, id_exchange: str, chart_id, symbol, interval):
        try:
            ...
            if ws:
                try:
                    cl.close()
                    del self.map_chart_exchange[
                        f"client-{chart_id}-{symbol}-{interval}"
                    ]
                except:
                    pass
