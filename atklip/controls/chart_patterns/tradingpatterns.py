...
def detect_head_shoulder(df, window=3):
    ...
    df.loc[mask_inv_head_shoulder, "head_shoulder_pattern"] = (
...
def detect_wedge(df, window=3):
    ...
    df["trend_high"] = (
        ...
        .apply(lambda x: 1 if (x[-1] - x[0]) > 0 else -1 if (x[-1] - x[0]) < 0 else 0)
    )
    df["trend_low"] = (
        df["Low"]
        .rolling(window=roll_window)
...
def detect_double_top_bottom(df, window=3, threshold=0.05):
    ...
    mask_double_top = (
        ...
        & (
            ...
            <= range_threshold * (df["High"].shift(1) + df["Low"].shift(1)) / 2
        )
        & (
            (df["High"].shift(-1) - df["Low"].shift(-1))
            <= range_threshold * (df["High"].shift(-1) + df["Low"].shift(-1)) / 2
