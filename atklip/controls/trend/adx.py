...
from atklip.controls.pandas_ta import adx
import numpy as np
import pandas as pd
...
class ADX(QObject):
    ...
    def __init__(self,_candles,dict_ta_params) -> None:   
        ...
        self.mamode:str = dict_ta_params.get("mamode","rma")
        self.lensig:int = dict_ta_params.get("lensig",14)
        self.adxr_length:int = dict_ta_params.get("adxr_length",2)
        self.scalar:int = dict_ta_params.get("scalar",100)
        self.tvmode:bool = dict_ta_params.get("tvmode",False)
        ...
        self._name = f"ADX {self.length} {self.mamode}"
    ...
    def change_input(self,candles=None,dict_ta_params: dict={}):
        ...
        if dict_ta_params != {}:
            ...
            ta_param = f"{obj_id}-{ta_name}-{self.length}-{self.mamode}"
    ...
    def paire_data(self,INDICATOR:pd.DataFrame|pd.Series):
        ...
        if isinstance(INDICATOR,pd.Series):
            ...
            for name in column_names:
                if name.__contains__("ADX"):
                    roc_name = name
            y_data = INDICATOR[roc_name]
        return y_data

    @staticmethod
    def calculate(df: pd.DataFrame,length,lensig=None, adxr_length=None, scalar=None, tvmode=None,mamode=None,drift=None,offset=None):
        df = df.copy()
        df = df.reset_index(drop=True)
        
        INDICATOR = adx(high=df["high"],
                        low=df["low"],
                        close=df["close"],
                        length=length,
                        lensig=lensig,
                        ...
                        scalar=scalar,
                        tvmode=tvmode,
                        mamode=mamode).dropna().round(6)
        
        adx_name = f"ADX_{lensig}"
        adxr_name = f"ADXR_{lensig}_{adxr_length}"
        dmp_name = f"DMP_{length}"
        dmn_name = f"DMN_{length}"
        

        y_data = INDICATOR[adx_name]
