...
def load_zz(list_zizgzag: list, candles: List[OHLCV], percent: float):
    ...
    _new_zz = [
        [candles[0].index, candles[0].low, "low"],
        [candles[0].index, candles[0].high, "high"],
    ]
...
def my_zigzag(
    list_zizgzag: list = [], candles: List[OHLCV] = None, percent: float = 0.5
):
    if list_zizgzag == []:
        list_zizgzag = [
            [candles[0].index, candles[0].low, "low"],
...
class ZIGZAG(QObject):
    ...
    def __init__(
        self, parent, _candles, legs, deviation, retrace=False, last_extreme=False
    ) -> None:
    ...
    def connect_signals(self):
        self._candles.sig_reset_all.connect(
            self.started_worker, Qt.ConnectionType.AutoConnection
        )
        self._candles.sig_update_candle.connect(
            self.update_worker, Qt.ConnectionType.AutoConnection
        )
        self._candles.sig_add_candle.connect(
            self.add_worker, Qt.ConnectionType.AutoConnection
        )
        self._candles.signal_delete.connect(self.signal_delete)
        self._candles.sig_add_historic.connect(
    ...
    @staticmethod
    def calculate(
        list_zizgzag, candles: List[OHLCV], process: str = "", deviation: float = 1
    ):
        if process == "add" or process == "update":
            list_zizgzag = update_zz(
                list_zizgzag=list_zizgzag, ohlcv=candles[-1], percent=deviation
            )
        elif process == "load":
            list_zizgzag = load_zz(
                list_zizgzag=list_zizgzag, candles=candles, percent=deviation
            )
        else:
            list_zizgzag = my_zigzag(
                list_zizgzag=list_zizgzag, candles=candles, percent=deviation
            )
