...
class SRLevel:
    def __init__(
        self, start_time, price, sr_type, timeframe, strength=1, ephemeral=False
    ):
...
def calculate_sr_levels(
    df, pivot_length=15, atr_length=20, timeframe="", min_strength=1
):
    sr_levels = []
...
def merge_sr_levels(all_levels, atr, min_strength=1):
    ...
    for level in all_levels:
        ...
        for existing in merged:
            if (
                abs(existing.price - level.price) < atr * 0.125
                and existing.sr_type == level.sr_type
            ):
                exists = True
                break
...
def multi_timeframe_sr(df, config):
    ...
    return merge_sr_levels(all_levels, atr.mean(), min_strength=config["min_strength"])
