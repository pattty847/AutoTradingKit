...
def trend_type_indicator(
    ...
    adx_df = ta.adx(
        high=df["high"], low=df["low"], close=df["close"], length=adx_len, talib=False
    )
    ...
    df["trend_type"] = np.where(
        df["sideways"],
        0,
        np.where(df["trend_up"], 2, np.where(df["trend_down"], -2, np.nan)),
    )
