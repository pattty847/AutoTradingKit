...
def calculate_trailing_stop(
    ...
    if lower is None or upper is None:
        raise ValueError(
            "Invalid band type or parameters resulting in None values for bands."
        )
...
def utbot_with_bb(
    ...
    # print("src--------------")
    # print(src)
    lower_band, middle_band, upper_band = calculate_bands(
        data, band_type, channel_length, StdDev
    )
    # print("lower_band--------------")
    # print(lower_band)
    barState, buyStop, sellStop = calculate_trailing_stop(
        data, lower_band, upper_band, atr_length, mult, wicks
    )
    # print("barState--------------")
    # print(barState)
    # Trade Conditions
...
class UTBOT_ALERT_WITH_BB(QObject):
    ...
    def first_gen_data(self):
        ...
        _df = pd.DataFrame({"index": _index, "long": _long, "short": _short})

        self.df = _df.iloc[
            self.atr_length + self.channel_length + self.atr_utbot_length :
        ]
    ...
    def add(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            self.is_current_update = False
            df: pd.DataFrame = self._candles.df.iloc[
                -int(
                    self.atr_length + self.channel_length + self.atr_utbot_length + 10
                ) :
            ]
