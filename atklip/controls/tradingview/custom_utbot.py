...
def old_utbot(
    ...
    mask_1 = (src > np.roll(xATRTrailingStop, 1)) & (
        np.roll(src, 1) > np.roll(xATRTrailingStop, 1)
    )
    mask_2 = (src < np.roll(xATRTrailingStop, 1)) & (
        np.roll(src, 1) < np.roll(xATRTrailingStop, 1)
    )
    mask_3 = src > np.roll(xATRTrailingStop, 1)

    xATRTrailingStop = np.where(
        mask_1, np.maximum(np.roll(xATRTrailingStop, 1), src - nLoss), xATRTrailingStop
    )
    xATRTrailingStop = np.where(
        mask_2, np.minimum(np.roll(xATRTrailingStop, 1), src + nLoss), xATRTrailingStop
    )
    xATRTrailingStop = np.where(mask_3, src - nLoss, xATRTrailingStop)

    mask_buy = (np.roll(src, 1) < xATRTrailingStop) & (
        src > np.roll(xATRTrailingStop, 1)
    )
    mask_sell = (np.roll(src, 1) > xATRTrailingStop) & (
        src < np.roll(xATRTrailingStop, 1)
    )
    ...
    _ema = (
        ma(name=ma_mode, source=dataframe["close"], length=ema_period, mamode=ema_mode)
        .dropna()
        .to_numpy()
    )
    ...
    buy_condition_utbot = (
        (xATRTrailingStop[-_leng:] > _ema[-_leng:])
        & (pos[-_leng:] > 0)
        & (src[-_leng:] > _ema[-_leng:])
    )
    sell_condition_utbot = (
        (xATRTrailingStop[-_leng:] < _ema[-_leng:])
        & (pos[-_leng:] < 0)
        & (src[-_leng:] < _ema[-_leng:])
    )
...
def utbot(
    ...
    ema = ma(
        name=ma_mode, source=dataframe["close"], length=ema_period, mamode=ema_mode
    ).to_numpy()
    ...
    kq = dataframe_result.iloc[max([ema_period, atr_period]) - 1 :]
...
class CUSTOM_UTBOT_ALERT(QObject):
    ...
    def first_gen_data(self):
        ...
        self.xdata, self.long, self.short = (
            self.df["index"].to_numpy(),
            self.df["long"].to_numpy(),
            self.df["short"].to_numpy(),
        )
    ...
    def add(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            self.is_current_update = False
            df_long: pd.DataFrame = self._candles.df.iloc[
                -int(self.atr_long_period + self.ema_long_period + 10) :
            ]
            df_short: pd.DataFrame = self._candles.df.iloc[
                -int(self.atr_short_period + self.ema_short_period + 10) :
            ]
    ...
    def update(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            ...
            self.df.loc[self.df.index[-1], ["index", "long", "short"]] = [
                new_candle.index,
                _long.iloc[-1],
                _short.iloc[-1],
            ]

            self.xdata[-1], self.long[-1], self.short[-1] = (
                new_candle.index,
                _long.iloc[-1],
                _short.iloc[-1],
            )
