...
def calculate_smc_logic(
    data, swing_length=10, atr_length=50, history_length=20, box_width=2.5
):
    """
    ...
    data["swing_high"] = (
        data["high"]
        .rolling(window=swing_length * 2 + 1, center=True)
        .apply(
    ...
    for i in range(len(data)):
        if not np.isnan(data["swing_high"].iloc[i]):
            add_zone(
                supply_zones, data["swing_high"].iloc[i], data["atr"].iloc[i], "supply"
            )
        if not np.isnan(data["swing_low"].iloc[i]):
            add_zone(
                demand_zones, data["swing_low"].iloc[i], data["atr"].iloc[i], "demand"
            )
