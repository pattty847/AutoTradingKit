...
def supertrend(df, Factor, Pd):
    ...
    for i in range(1, len(df)):
        TrendUp[i] = (
            max(Up[i], TrendUp[i - 1]) if df["close"][i - 1] > TrendUp[i - 1] else Up[i]
        )
        TrendDown[i] = (
            min(Dn[i], TrendDown[i - 1])
            if df["close"][i - 1] < TrendDown[i - 1]
            else Dn[i]
        )
...
df = pd.read_csv(
    "your_data.csv", parse_dates=True, index_col="date"
