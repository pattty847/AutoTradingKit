...
def support_resistance_with_breaks(df,left_bars = 15,
                                    right_bars = 15,
                                    volume_thresh = 20):
    ...
    df['break_down'] = (df['close'] < df['low_use_pivot']) & (df['osc'] > volume_thresh) & (df['open'] - df['close'] < df['high'] - df['open'])
    df['break_up'] = (df['close'] > df['high_use_pivot']) & (df['osc'] > volume_thresh) & (df['open'] - df['low'] > df['close'] - df['open'])

    # Conditions for bull / bear wicks
    df['bull_wick'] = (df['close'] > df['high_use_pivot']) & (df['open'] - df['low'] > df['close'] - df['open'])
    df['bear_wick'] = (df['close'] < df['low_use_pivot']) & (df['open'] - df['close'] < df['high'] - df['open'])

    # Alert conditions
    df['alert_support_broken'] = crossunder(df['close'], df['low_use_pivot']) & (df['osc'] > volume_thresh)
    df['alert_resistance_broken'] = crossover(df['close'], df['high_use_pivot']) & (df['osc'] > volume_thresh)

    return df


def calculate_support_resistance(df, left_bars=15, right_bars=15, volume_thresh=20, toggle_breaks=True):
    """
    ...
    df['pivot_high'] = df['high'].rolling(window=left_bars + right_bars + 1).apply(
        lambda x: x[left_bars] if x[left_bars] == max(x) else np.nan, raw=True
    )
    df['pivot_low'] = df['low'].rolling(window=left_bars + right_bars + 1).apply(
        lambda x: x[left_bars] if x[left_bars] == min(x) else np.nan, raw=True
    )
    ...
    if toggle_breaks:
        df['break_support'] = crossunder(df['close'], df['pivot_low'].shift(right_bars + 1)) & \
                              ~(df['open'] - df['close'] < df['high'] - df['open']) & \
                              (df['osc'] > volume_thresh)
        df['break_resistance'] = crossover(df['close'], df['pivot_high'].shift(right_bars + 1)) & \
                                 ~(df['open'] - df['low'] > df['close'] - df['open']) & \
                                 (df['osc'] > volume_thresh)
    
    # Tính toán Bull/Bear Wicks
    if toggle_breaks:
        df['bull_wick'] = crossover(df['close'], df['pivot_high'].shift(right_bars + 1)) & \
                           (df['open'] - df['low'] > df['close'] - df['open'])
        df['bear_wick'] = crossunder(df['close'], df['pivot_low'].shift(right_bars + 1)) & \
                           (df['open'] - df['close'] < df['high'] - df['open'])
    
    # Tính toán các điều kiện cảnh báo
    df['alert_support_broken'] = crossunder(df['close'], df['pivot_low'].shift(right_bars + 1)) & \
                                 (df['osc'] > volume_thresh)
    df['alert_resistance_broken'] = crossover(df['close'], df['pivot_high'].shift(right_bars + 1)) & \
                                    (df['osc'] > volume_thresh)
...
class SuperTrendWithStopLoss(QObject):
    ...
    @staticmethod
    def calculate(df: pd.DataFrame, left_bars: int = 15, right_bars: int = 15, volume_thresh: int = 20):
        df = df.copy()
        _index = df["index"]
        df = df.reset_index(drop=True)
        
        INDICATOR = support_resistance_with_breaks(df,
                                                   left_bars = left_bars,
