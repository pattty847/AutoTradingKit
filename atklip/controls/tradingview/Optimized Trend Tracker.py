...
def calculate_OTT(df, src="close", length=2, percent=1.4, mav="VAR"):
    ...
    if mav == "SMA":
        ...
        for i in range(len(df)):
            if i == 0:
                ...
                VAR[i] = (
                    valpha * abs(vCMO.iloc[i]) * df[src].iloc[i]
                    + (1 - valpha * abs(vCMO.iloc[i])) * prev_VAR
                )
        df["MAvg"] = VAR
    elif mav == "WWMA":
        alpha = 1 / length
    ...
    for i in range(len(df)):
        if i == 0:
            ...
            # Update longStop
            prev_long = (
                longStop[i - 1] if not np.isnan(longStop[i - 1]) else longStop[i - 1]
            )
            ...
            # Update shortStop
            prev_short = (
                shortStop[i - 1] if not np.isnan(shortStop[i - 1]) else shortStop[i - 1]
            )
