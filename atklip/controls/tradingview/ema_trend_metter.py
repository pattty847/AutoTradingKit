...
def ema_trend_metter(
    df: pd.DataFrame,
    base_ema_length: int = 13,
    ema_length_1: int = 21,
    ema_length_2: int = 34,
    ema_length_3: int = 55,
) -> pd.DataFrame:
...
class EMA_TREND_METTER(QObject):
    ...
    def __init__(self, _candles, dict_ta_params) -> None:
        ...
        self.xdata, self.uptrend, self.downtrend = (
            np.array([]),
            np.array([]),
            np.array([]),
        )
    ...
    @staticmethod
    def calculate(
        df: pd.DataFrame, base_ema_length, ema_length_1, ema_length_2, ema_length_3
    ):
        df = df.copy()
        df = df.reset_index(drop=True)

        INDICATOR = ema_trend_metter(
            df=df,
            base_ema_length=base_ema_length,
            ema_length_1=ema_length_1,
            ema_length_2=ema_length_2,
            ema_length_3=ema_length_3,
        ).dropna()
    ...
    def callback_update(self, future: Future):
        ...
        last_uptrend = df["uptrend"].iloc[-1]
        last_downtrend = df["downtrend"].iloc[-1]
        self.df.loc[self.df.index[-1], ["index", "uptrend", "downtrend"]] = [
            last_index,
            last_uptrend,
            last_downtrend,
        ]
        self.xdata[-1], self.uptrend[-1], self.downtrend[-1] = (
            last_index,
            last_uptrend,
            last_downtrend,
        )
