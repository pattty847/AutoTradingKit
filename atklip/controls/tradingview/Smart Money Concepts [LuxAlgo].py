...
def getCurrentStructure(
    ...
    if newPivot:
        if pivotLow:
            pivot = (
                equalLow if equalHighLow else (internalLow if internal else swingLow)
            )
            ...
            pivot = (
                equalHigh if equalHighLow else (internalHigh if internal else swingHigh)
            )
            pivot.lastLevel = pivot.currentLevel
...
def displayStructure(
    internal=False, close=None, high=None, low=None, time=None, bar_index=None
):
...
def updateTrailingExtremes(high, low, time):
    ...
    trailing.lastBottomTime = (
        time if trailing.bottom == low else trailing.lastBottomTime
    )
...
def process_data(df):
    ...
    for i in range(len(df)):
        getCurrentStructure(
            5, False, False, df["high"], df["low"], df["time"], df.index
        )
        displayStructure(
            False, df["close"], df["high"], df["low"], df["time"], df.index
        )
