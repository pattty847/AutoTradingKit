...
def chandelier_exit(
    ...
    """
    # Calculate ATR
    data["atr"] = atr_multiplier * ta.atr(
    ...
    data["long_stop"] = (
        data["highest_close"] if use_close else data["highest_high"]
    ) - data["atr"]
    data["long_stop_prev"] = data["long_stop"].shift(1)
    data["long_stop"] = np.where(
        data["close"].shift(1) > data["long_stop_prev"],
    ...
    data["short_stop"] = (
        data["lowest_close"] if use_close else data["lowest_low"]
    ) + data["atr"]
    data["short_stop_prev"] = data["short_stop"].shift(1)
    data["short_stop"] = np.where(
        data["close"].shift(1) < data["short_stop_prev"],
    ...
    data["dir"] = np.where(
        data["close"] > data["short_stop_prev"],
        1,
    ...
    # Final result
    result = data[
        ["atr", "long_stop", "short_stop", "dir", "buy_signal", "sell_signal"]
    ]
    return result
