...
class RVGI(QObject):
    ...
    def calculate(df: pd.DataFrame, length, swma_length):
        ...
        return pd.DataFrame(
            {"index": _index, "rvgi": rvgi_.tail(_len), "signalma": signalma.tail(_len)}
        )
    ...
    def first_gen_data(self):
        ...
        process = HeavyProcess(
            self.calculate, self.callback_first_gen, df, self.length, self.swma_length
        )
        process.start()
    ...
    def add(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            df: pd.DataFrame = self._candles.get_df(
                (self.length + self.swma_length) * 2
            )
            process = HeavyProcess(
                self.calculate, self.callback_add, df, self.length, self.swma_length
            )
            process.start()
        else:
            pass
    ...
    def update(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            ...
            process = HeavyProcess(
                self.calculate, self.callback_update, df, self.length, self.swma_length
            )
            process.start()
        else:
            pass
    ...
    def callback_first_gen(self, future: Future):
        ...
        self.xdata, self.rvgi_, self.signalma = (
    ...
    def callback_add(self, future: Future):
        ...
        new_frame = pd.DataFrame(
            {"index": [last_index], "rvgi": [last_rvgi], "signalma": [last_signalma]}
        )
    ...
    def callback_update(self, future: Future):
        ...
        last_rvgi = df["rvgi"].iloc[-1]
        last_signalma = df["signalma"].iloc[-1]
        self.df.loc[self.df.index[-1], ["index", "rvgi", "signalma"]] = [
            last_index,
            last_rvgi,
            last_signalma,
        ]
        self.xdata[-1], self.rvgi_[-1], self.signalma[-1] = (
            last_index,
            last_rvgi,
            last_signalma,
        )
