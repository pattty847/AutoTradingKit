...
from atklip.controls.pandas_ta.momentum import trix
import numpy as np
import pandas as pd
...
class TRIX(QObject):
    ...
    @staticmethod
    def calculate(
        df: pd.DataFrame, source, length_period, signal_period, mamode, drift, offset
    ):
        df = df.copy()
        ...
        return pd.DataFrame(
            {"index": _index, "trix": trix_.tail(_len), "signalma": signalma.tail(_len)}
        )
    ...
    def callback_add(self, future: Future):
        ...
        new_frame = pd.DataFrame(
            {"index": [last_index], "trix": [last_trix], "signalma": [last_signalma]}
        )
    ...
    def callback_update(self, future: Future):
        ...
        last_trix = df["trix"].iloc[-1]
        last_signalma = df["signalma"].iloc[-1]
        self.df.loc[self.df.index[-1], ["index", "trix", "signalma"]] = [
            last_index,
            last_trix,
            last_signalma,
        ]
        self.xdata[-1], self.trix_[-1], self.signalma[-1] = (
            last_index,
            last_trix,
            last_signalma,
        )
