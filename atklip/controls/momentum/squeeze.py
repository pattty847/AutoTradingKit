...
class SQEEZE(QObject):
    ...
    def __init__(self, _candles, dict_ta_params: dict = {}) -> None:
        ...
        self.kwargs = {
            "use_tr": dict_ta_params.get("use_tr", True),
    ...
    def calculate(
        ...
        return pd.DataFrame({"index": _index, "SQZ_data": SQZ_data.tail(_len)})
    ...
    def add(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            df: pd.DataFrame = self._candles.get_df(
                int(self.bb_length + self.kc_length + self.mom_length + self.mom_smooth)
                + 10
            )
            process = HeavyProcess(
                self.calculate,
    ...
    def callback_first_gen(self, future: Future):
        ...
        self.xdata, self.SQZ_data = (
            self.df["index"].to_numpy(),
            self.df["SQZ_data"].to_numpy(),
        )
    ...
    def callback_add(self, future: Future):
        ...
        new_frame = pd.DataFrame({"index": [last_index], "SQZ_data": [last_SQZ_data]})
    ...
    def callback_update(self, future: Future):
        ...
        last_index = df["index"].iloc[-1]
        last_SQZ_data = df["SQZ_data"].iloc[-1]
        self.df.loc[self.df.index[-1], ["index", "SQZ_data"]] = [
            last_index,
            last_SQZ_data,
        ]
