...
class TSI(QObject):
    ...
    def calculate(
        ...
        return pd.DataFrame(
            {"index": _index, "tsi": tsi_.tail(_len), "signalma": signalma.tail(_len)}
        )
    ...
    def callback_add(self, future: Future):
        ...
        new_frame = pd.DataFrame(
            {"index": [last_index], "tsi": [last_tsi], "signalma": [last_signalma]}
        )
    ...
    def callback_update(self, future: Future):
        ...
        last_tsi = df["tsi"].iloc[-1]
        last_signalma = df["signalma"].iloc[-1]
        self.df.loc[self.df.index[-1], ["index", "tsi", "signalma"]] = [
            last_index,
            last_tsi,
            last_signalma,
        ]
        self.xdata[-1], self.tsi_[-1], self.signalma[-1] = (
            last_index,
            last_tsi,
            last_signalma,
        )
