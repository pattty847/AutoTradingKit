...
class MACD(QObject):
    ...
    def get_data(self, start: int = 0, stop: int = 0):
        ...
        if start == 0 and stop == 0:
            ...
            macd_data, signalma, histogram = (
                self.macd_data[start:stop],
                self.signalma[start:stop],
                self.histogram[start:stop],
            )
    ...
    def calculate(
        ...
        _len = min(
            [len(histogram), len(macd_data), len(signalma), int(len(df) - slow_period)]
        )
    ...
    def callback_update(self, future: Future):
        ...
        self.df.loc[self.df.index[-1], ["index", "macd", "histogram", "signalma"]] = [
            last_index,
            last_macd,
            last_histogram,
            last_signalma,
        ]
        self.xdata[-1], self.macd_data[-1], self.histogram[-1], self.signalma[-1] = (
            last_index,
            last_macd,
            last_histogram,
            last_signalma,
