...
from atklip.controls.pandas_ta.momentum import cci
import numpy as np
import pandas as pd
...
class CCI(QObject):
    ...
    def calculate(df: pd.DataFrame, length, c):
        ...
        INDICATOR = (
            cci(high=df["high"], low=df["low"], close=df["close"], length=length, c=c)
            .dropna()
            .round(6)
        )
        if isinstance(INDICATOR, pd.Series):
    ...
    def first_gen_data(self):
        ...
        process = HeavyProcess(
            self.calculate, self.callback_first_gen, df, self.length, self.c
        )
        process.start()
    ...
    def add_historic(self, n: int):
        ...
        process = HeavyProcess(
            self.calculate, self.callback_gen_historic_data, df, self.length, self.c
        )
        process.start()
    ...
    def add(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            ...
            process = HeavyProcess(
                self.calculate, self.callback_add, df, self.length, self.c
            )
            process.start()
        else:
            pass
    ...
    def update(self, new_candles: List[OHLCV]):
        ...
        if (self.first_gen == True) and (self.is_genering == False):
            df: pd.DataFrame = self._candles.get_df(self.length * 5)
            process = HeavyProcess(
                self.calculate, self.callback_update, df, self.length, self.c
            )
            process.start()
        else:
            pass
