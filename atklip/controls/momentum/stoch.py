...
class STOCH(QObject):
    ...
    @staticmethod
    def calculate(
        df: pd.DataFrame, smooth_k_period, k_period, d_period, mamode, offset
    ):
        df = df.copy()
    ...
    def callback_add(self, future: Future):
        ...
        new_frame = pd.DataFrame(
            {"index": [last_index], "stoch": [last_stoch], "signalma": [last_signalma]}
        )
    ...
    def callback_update(self, future: Future):
        ...
        last_signalma = df["signalma"].iloc[-1]
        last_stoch = df["stoch"].iloc[-1]
        self.df.loc[self.df.index[-1], ["index", "stoch", "signalma"]] = [
            last_index,
            last_stoch,
            last_signalma,
        ]
        self.xdata[-1], self.stoch_[-1], self.signalma[-1] = (
            last_index,
            last_stoch,
            last_signalma,
        )
