...
class STOCHRSI(QObject):
    ...
    @staticmethod
    def calculate(
        df: pd.DataFrame, source, period, rsi_period, k_period, d_period, mamode, offset
    ):
        df = df.copy()
    ...
    def callback_update(self, future: Future):
        ...
        last_stochrsi = df["stochrsi"].iloc[-1]
        last_signalma = df["signalma"].iloc[-1]
        self.df.loc[self.df.index[-1], ["index", "stochrsi", "signalma"]] = [
            last_index,
            last_stochrsi,
            last_signalma,
        ]
        self.xdata[-1], self.stochrsi_[-1], self.signalma[-1] = (
            last_index,
            last_stochrsi,
            last_signalma,
        )
