...
class STC(QObject):
    ...
    def __init__(self, _candles, dict_ta_params) -> None:
        ...
        self._name = (
            f"STC {self.source} {self.slow} {self.fast} {self.tclength} {self.mamode}"
        )
    ...
    def get_data(self, start: int = 0, stop: int = 0):
        ...
        if start == 0 and stop == 0:
            ...
            stc_, macd, stoch = (
                self.stc_[start:stop],
                self.macd[start:stop],
                self.stoch[start:stop],
            )
        return x_data, stc_, macd, stoch
    ...
    @staticmethod
    def calculate(
        df: pd.DataFrame, source, tclength, fast, slow, mamode, factor, offset
    ):
        df = df.copy()
    ...
    def callback_update(self, future: Future):
        ...
        last_macd = df["macd"].iloc[-1]
        last_stoch = df["stoch"].iloc[-1]
        self.df.loc[self.df.index[-1], ["index", "stc", "macd", "stoch"]] = [
            last_index,
            last_stc,
            last_macd,
            last_stoch,
        ]
        self.xdata[-1], self.stc_[-1], self.macd[-1], self.stoch[-1] = (
            last_index,
            last_stc,
            last_macd,
            last_stoch,
