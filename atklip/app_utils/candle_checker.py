...
def check_candle_notail_bullish_with_long_body(
    open, high, low, close, percent: float = 0.5
):
    ...
    if height == 0:
        return False
    return (
        check_candle_notail_bullish(open, high, low, close)
        and above_bd / height >= percent
    )


def check_candle_nohead_bearish_with_long_body(
    open, high, low, close, percent: float = 0.5
):
    ...
    if height == 0:
        return False
    return (
        check_candle_nohead_bearish(open, high, low, close)
        and is_bearish(open, close)
        and below_bd / height >= percent
    )
