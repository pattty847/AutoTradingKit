...
from pyqtgraph import GraphicsView, GraphicsLayout, setConfigOption
from pyqtgraph.dockarea import DockArea, Dock, DockLabel
...
class CustomDockArea(DockArea):
    def __init__(self, parent=None, temporary=False, home=None):
        super(CustomDockArea, self).__init__(
            parent=parent, temporary=temporary, home=home
        )
    ...
    def addDock(self, dock=None, position="bottom", relativeTo=None, **kwds):
        ...
        if self._parent.chart is not None and not isinstance(_dock.panel, Chart):
            ...
            self._parent.chart.crosshair_x_value_change.disconnect(
                _dock.panel.xAxis.change_value
            )
...
class CustomDock(Dock):
    ...
    def __init__(
        ...
        **kargs,
        ...
        super(CustomDock, self).__init__(
            name,
            area=area,
            size=size,
            widget=widget,
    ...
    def addWidget(
        self,
        widget: QWidget | Chart | SubChart | ViewSubPanel,
        row=None,
        col=0,
        rowspan=1,
        colspan=1,
    ...
    def float(self):
        if self.panel is not None and self.chart is not None:
            ...
            if not isinstance(self.panel, SubChart):
                self.panel.Chart.crosshair_x_value_change.connect(
                    slot=self.panel.xAxis.change_value,
...
class GraphSplitter(ViewSplitter):
    ...
    def __init__(self, parent: QWidget | None = ...) -> None:
        super().__init__(parent)
        self.stratygies: dict = {
            IndicatorType.ATKBOT_SUPERTREND_SSCANDLE: {
        ...
        self.sig_add_sub_panel.connect(
            self.add_sub_panel, Qt.ConnectionType.AutoConnection
        )
    ...
    def setup_chart(
        ...
        self.chart = Chart(
            parent=self,
            exchange_name=current_ex,
            symbol=current_symbol,
            interval=curent_interval,
        )
        Signal_Proxy(
