...
class INTERVALS:
    ...
    def __init__(self, chart: object):
        ...
        for interval in self.intervals:
            ...
            self.map_intervals_indicator[interval] = ExchangeModel(
                interval, jp_cdl, heikin_cdl, None, None, None, None, [], False, 2, 3
            )
        ...
        if self.keys == None:
            ...
            AppConfig.sig_set_single_data.emit(
                (
                    f"app.keys",
                    {self.exchange_name: {"apikey": api, "secretkey": secretkey}},
                )
            )
            self.keys = AppConfig.get_config_value(f"app.keys")
    ...
    def get_jp_heikin_cdls_for_indicator(
        self, interval: str = "1m", indicator: object = None
    ):
        Exchangedata: ExchangeModel = self.map_intervals_indicator.get(interval, None)
        if not Exchangedata:
            Exchangedata = self.map_intervals_indicator[interval] = ExchangeModel(
                interval, jp_cdl, heikin_cdl, None, None, None, None, [], False, 2, 3
            )
    ...
    def update_exchange_status(self, interval: str = "1m"):
        ...
        asyncio.run(
            self.Exchange_Manager.remove_exchange(
                self.exchange_name, self.chart.id, self.symbol, interval
            )
        )
    ...
    def set_up_exchange(self, ExchangeModel: ExchangeModel):
        ...
        ExchangeModel.client_exchange, ExchangeModel.ws_exchange = (
            self.Exchange_Manager.add_exchange(
                self.exchange_name,
                self.chart.id,
                self.symbol,
                interval,
    ...
    async def reload_market(self, ExchangeModel: ExchangeModel):
        interval: str = ExchangeModel.interval
        ExchangeModel.worker_reload = asyncio.create_task(
            self.Exchange_Manager.reload_markets(
                self.exchange_name, self.chart.id, self.symbol, interval
            )
        )
    ...
    def set_market_by_symbol(self, ExchangeModel: ExchangeModel):
        ...
        ExchangeModel.quanty_precision = convert_precision(
            ...
            if client_socket != None and ws_socket != None:
                ...
                if len(_ohlcv) >= 2:
                    pre_ohlcv = OHLCV(
                        ...
                        _ohlcv[-2][4],
                        round(
                            (_ohlcv[-2][2] + _ohlcv[-2][3]) / 2, ExchangeModel.precision
                        ),
                        round(
                            (_ohlcv[-2][2] + _ohlcv[-2][3] + _ohlcv[-2][4]) / 3,
                            ExchangeModel.precision,
                        ),
                        round(
                            (
                                _ohlcv[-2][1]
                    ...
                    last_ohlcv = OHLCV(
                        ...
                        _ohlcv[-1][4],
                        round(
                            (_ohlcv[-1][2] + _ohlcv[-1][3]) / 2, ExchangeModel.precision
                        ),
                        round(
                    ...
                    _is_add_candle = ExchangeModel.japan_cdl.update(
                        [pre_ohlcv, last_ohlcv]
                    )
                    ExchangeModel.heikin_cdl.update(
                        ExchangeModel.japan_cdl.candles[-2:], _is_add_candle
                    )
