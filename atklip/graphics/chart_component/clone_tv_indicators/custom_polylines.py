...
class BasePolyLine(SpecialROI):
    ...
    **Arguments**
    ...
    def addSegment(self, h1, h2, index=None):
        seg = _FiboLineSegment(
            handles=(h1, h2),
            pen=mkPen(color="#9598a1", width=1, style=Qt.DashLine),
        ...
        for h in seg.handles:
            ...
            h["item"].setAcceptedMouseButtons(
                h["item"].acceptedMouseButtons() | Qt.MouseButton.LeftButton
            )  ## have these handles take left clicks too, so that handles cannot be added on top of other handles
...
class RangePolyLine(BasePolyLine):  # for date price range
    ...
    def addSegment(self, h1, h2, index=None):
        ...
        self.v_arrow = ArrowItem(
            ...
            tailWidth=1,
            pen=None,
            brush=draw_line_color,
        )
        self.h_arrow = ArrowItem(
            angle=180,
            tipAngle=30,
            baseAngle=20,
            headLen=10,
    ...
    def movePoint(
        self, handle, pos, modifiers=Qt.KeyboardModifier, finish=True, coords="parent"
    ):
    ...
    def paint(self, p: QPainter, *args):
        ...
        if self.handles:
            ...
            diff = h1 - h0
            p.drawLine(
                QPointF(h1.x(), h1.y() - diff.y() / 2),
                QPointF(h0.x(), h1.y() - diff.y() / 2),
            )
            p.drawLine(
                QPointF(h1.x() - diff.x() / 2, h1.y()),
                QPointF(h1.x() - diff.x() / 2, h0.y()),
            )
...
class MouseDragHandler(object):
    ...
    def mouseDragEvent(self, ev):
        ...
        if self.dragMode == "translate":
            ...
            roi.setSize(
                Point(self.startState["size"]) * diff,
                centerLocal=ev.buttonDownPos(),
                snap=snap,
                finish=False,
            )
