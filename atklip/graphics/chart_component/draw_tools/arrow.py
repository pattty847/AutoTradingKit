...
class Arrow(QGraphicsObject):
    ...
    def __init__(
        self, chart, _type, high_color: str = "#089981", low_color: str = "#f23645"
    ) -> None:
        ...
        if mamode != None:
            ...
            self.has: dict = {
                ...
                "y_axis_show": True,
                "inputs": {"source": self.source, "show": True},
                "styles": {
        ...
        if not isinstance(self.source, JAPAN_CANDLE) and not isinstance(
            self.source, HEIKINASHI
        ):
        ...
        self.source.sig_reset_all.connect(
            self.update_source, Qt.ConnectionType.AutoConnection
        )

        self.source.sig_update_candle.connect(
            self.update_asyncworker, Qt.ConnectionType.AutoConnection
        )
        self.source.sig_add_candle.connect(
            self.update_asyncworker, Qt.ConnectionType.AutoConnection
        )
        self.source.sig_add_historic.connect(
            self.threadpool_asyncworker, Qt.ConnectionType.AutoConnection
        )
    ...
    def update_source(self):
        ...
        if isinstance(self.source, N_SMOOTH_CANDLE):
            self.has["name"] = (
                f"""{source_name} {self.has["inputs"]["mamode"].name} {self.has["inputs"]["ma_period"]} {self.has["inputs"]["n_smooth_period"]}"""
            )
        if isinstance(self.source, SMOOTH_CANDLE):
            self.has["name"] = (
                f"""{source_name} {self.has["inputs"]["mamode"].name} {self.has["inputs"]["ma_period"]}"""
            )
        else:
            self.has["name"] = (
                f"""{source_name} {self.chart.symbol} {self.chart.interval}"""
            )
    ...
    def update_inputs(self, _input, _source):
        ...
        if update:
            ...
            if mamode != None:
                if isinstance(self.source, N_SMOOTH_CANDLE):
                    self.has["name"] = (
                        f"""{self.source.source_name} {self.has["inputs"]["mamode"].name} {self.has["inputs"]["ma_period"]} {self.has["inputs"]["n_smooth_period"]}"""
                    )
                else:
                    self.has["name"] = (
                        f"""{self.source.source_name} {self.has["inputs"]["mamode"].name} {self.has["inputs"]["ma_period"]}"""
                    )
            else:
                self.has.update({"inputs": {"source": self.source, "show": False}})
    ...
    def get_source(
        self,
        _type: IndicatorType,
        mamode: PD_MAType = PD_MAType.EMA,
        period: int = 3,
        n: int = 3,
    ):
        ...
        if _type.value == "japan" or _type.value == "Sub_Chart":
            ...
            smooth_jp_candle = SMOOTH_CANDLE(
                self.precision, self.chart.jp_candle, mamode, period
            )
            smooth_jp_candle._source_name = (
                f"sm_jp {self.chart.symbol} {self.chart.interval}"
            )
            ...
            n_smooth_jp = N_SMOOTH_CANDLE(
                self.precision, self.chart.jp_candle, n, mamode, period
            )
            n_smooth_jp._source_name = (
                f"n_smooth_jp {self.chart.symbol} {self.chart.interval}"
            )
            ...
            smooth_heikin = SMOOTH_CANDLE(
                self.precision, self.chart.heikinashi, mamode, period
            )
            smooth_heikin._source_name = (
                f"sm_heikin {self.chart.symbol} {self.chart.interval}"
            )
            ...
            n_smooth_heikin = N_SMOOTH_CANDLE(
                self.precision, self.chart.heikinashi, n, mamode, period
            )
            n_smooth_heikin._source_name = (
                f"n_smooth_heikin {self.chart.symbol} {self.chart.interval}"
            )
    ...
    def get_styles(self):
        styles = {
            "pen_highcolor": self.has["styles"]["pen_highcolor"],
    ...
    def threadpool_asyncworker(self, candles=None):
        ...
        self.worker.signals.setdata.connect(
            self.setData, Qt.ConnectionType.AutoConnection
        )
        self.worker.start()
    ...
    def update_asyncworker(self, candles=None):
        ...
        self.worker = FastWorker(self.update_last_data, candles)
        self.worker.signals.setdata.connect(
            self.updateData, Qt.ConnectionType.AutoConnection
        )
        self.worker.start()
    ...
    def paint(
        self, painter: QPainter, opt: QStyleOptionGraphicsItem, w: QWidget
    ) -> None:
        """
    ...
    def draw_single_candle(self, painter: QPainter, _open, _max, _min, close, w, t):
        ...
        points = QPolygonF(
            [
                QPointF(
                    start_x + tail_length / 2, start_y - triangle_width / 2
                ),  # Góc trên
                QPointF(
                    start_x + tail_length / 2, start_y + triangle_width / 2
                ),  # Góc dưới
                QPointF(
                    start_x + tail_length / 2 + triangle_length, start_y
                ),  # Đỉnh nhọn
            ]
        )
        painter.drawPolygon(points)
