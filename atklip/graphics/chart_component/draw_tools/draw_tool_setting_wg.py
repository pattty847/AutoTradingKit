...
from atklip.gui.qfluentwidgets.components.widgets.menu import (
    CheckableMenu,
    MenuIndicatorType,
)
...
class PopUpSettingMenu(CommandBarView):
            ...
            for _input in list(styles.keys()):
                if "pen" in _input:
                    ...
                    if styles[_input] == True:
                        ...
                        lock.installEventFilter(
                            ToolTipFilter(lock, 3000, ToolTipPosition.TOP)
                        )
                    ...
                    if styles[_input] == True:
                        ...
                        setting.installEventFilter(
                            ToolTipFilter(setting, 3000, ToolTipPosition.TOP)
                        )
                    ...
                    if styles[_input] == True:
                        ...
                        delete.installEventFilter(
                            ToolTipFilter(delete, 3000, ToolTipPosition.TOP)
                        )
    ...
    def open_tool_setting_menu(self):
        if self._menu is None:
            ...
            try:
                ...
                self._menu = DrawSettingMenu(
                    self.tool, self.chart.mainwindow, self.chart
                )
                _x = self.chart.width()
                _y = self.chart.height()
                x = (_x - self._menu.width()) / 2
    ...
    def createCheckableMenu(self, pos=None):
        ...
        menu.addActions(
            [createTimeAction, shootTimeAction, modifiedTimeAction, nameAction]
        )
        menu.addSeparator()
