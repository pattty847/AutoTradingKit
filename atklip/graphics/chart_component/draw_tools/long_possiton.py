...
from atklip.app_utils import (
    percent_caculator,
    calculate_pl_with_fees,
    calculate_recommended_capital_base_on_risk,
...
class Longposition(BaseRect):
    ...
    def __init__(
        ...
        **args,
        ...
        self.under_part.addScaleHandle(
            [1, 0], [0, 0], lockAspect=True, item=self.center_handle
        )
        self.under_handle = self.under_part.addScaleHandle(
            [0, 0], [0, 1], lockAspect=True
        )
    ...
    def update_text(self):
        ...
        point0_under_part_ = self.mapToParent(
            self.under_part.mapToParent(Point(h0_under_part))
        )
        ...
        if self.has["inputs"]["is_risk"]:
            recom_capital = calculate_recommended_capital_base_on_risk(
            ...
            recom_capital = calculate_recommended_capital_base_on_loss_capital(
                entry_price=entry_price,
                stop_loss_price=stop_loss_price,
        ...
        stoploss = calculate_pl_with_fees(
            entry_price=entry_price,
    ...
    def get_styles(self):
        styles = {
            "above_brush": self.has["styles"]["above_brush"],
    ...
    def boundingRect(self):
        ...
        return QRectF(
            0,
            -self.under_part.state["size"][1],
            self.state["size"][0],
            self.state["size"][1] + self.under_part.state["size"][1],
        )  # .normalized()
