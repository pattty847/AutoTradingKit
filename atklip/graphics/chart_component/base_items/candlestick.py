...
class CandleStick(GraphicsObject):
    ...
    def __init__(
        ...
        if not (
            isinstance(self.source, JAPAN_CANDLE) or isinstance(self.source, HEIKINASHI)
        ):
    ...
    def get_source(
        ...
        if _type == "japan" or _type == "Sub_Chart":
            ...
            candle = N_SMOOTH_CANDLE(
                self.chart, self.chart.jp_candle, n, mamode, period
            )
            candle._source_name = (
            ...
            candle = N_SMOOTH_CANDLE(
                self.chart, self.chart.heikinashi, n, mamode, period
            )
            candle._source_name = (
    ...
    def threadpool_asyncworker(self, candles=None | bool | list | int):
        ...
        self.worker.signals.finished.connect(
            self.set_price_line, Qt.ConnectionType.AutoConnection
        )
        self.worker.start()
    ...
    def get_yaxis_param(self):
        if len(self.source.candles) > 0:
            ...
            colorline = (
                "#089981" if last_close_price_ >= last_open_price_ else "#f23645"
            )
    ...
    def draw_single_candle(self, _open, _max, _min, close, w, t):
        ...
        if _open > close:
            self.outline_pen = mkPen(
                color=self.has["styles"]["pen_lowcolor"], width=0.7
            )
            ...
            self.outline_pen = mkPen(
                color=self.has["styles"]["pen_highcolor"], width=0.7
            )
            p.setPen(self.outline_pen)
    ...
    def setData(self, data) -> None:
        ...
        [
            self.draw_candle(
                _open[index], _max[index], _min[index], close[index], w, x_data[index]
            )
    ...
    def updateData(self, data) -> None:
        ...
        pre_t = x_data[-2]
        t = x_data[-1]
        pre_open, pre_max, pre_min, pre_close = (
            y_data[0][-2],
            y_data[1][-2],
            y_data[2][-2],
            y_data[3][-2],
        )
        _open, _max, _min, _close = (
            y_data[0][-1],
            y_data[1][-1],
            y_data[2][-1],
            y_data[3][-1],
