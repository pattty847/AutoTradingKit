...
class IndicatorPanel(QWidget):
    ...
    def _on_hide_show(self):
        if self.indicator.isVisible():
            self.indicator.hide()
            icon_path = (
                FIF.EYE_CLOSE.path(Theme.DARK)
                if isDarkTheme()
                else FIF.EYE_CLOSE.path(Theme.LIGHT)
            )
            ...
            self.indicator.show()
            icon_path = (
                FIF.EYE_OPEN.path(Theme.DARK)
                if isDarkTheme()
                else FIF.EYE_OPEN.path(Theme.LIGHT)
            )
            self.btn_show_hide.setIcon(QIcon(icon_path))
    ...
    def show_menu(self) -> None:
        if self._menu is None:
            ...
            try:
                ...
                self._menu = IndicatorSettingMenu(
                    self.indicator, self._parent, self.chart
                )
                _x = self._parent.width()
                _y = self._parent.height()
                x = (_x - self._menu.width()) / 2
