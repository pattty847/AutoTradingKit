...
class Chart(ViewPlotWidget):
    def __init__(
        ...
        self.sig_reload_indicator_panel.connect(
            self._parent.reload_pre_indicator, Qt.ConnectionType.AutoConnection
        )
        self.apikey = apikey
        ...
        self.sig_add_indicator_panel.connect(
            self.setup_indicator, Qt.ConnectionType.AutoConnection
        )
        self.first_run.connect(
            self.set_data_dataconnect, Qt.ConnectionType.AutoConnection
        )
    ...
    def get_api_secret_key(self, exchange_name) -> tuple:
        ...
        if keys == None:
            ...
            AppConfig.sig_set_single_data.emit(
                (f"app.keys.{exchange_name}", {"apikey": api, "secretkey": secretkey})
            )
            keys = AppConfig.get_config_value(f"app.keys")
        ...
        if api_secret_keys is None:
            ...
            keys = AppConfig.get_config_value(f"app.keys")
        apikey, secretkey = (
            keys[exchange_name]["apikey"],
            keys[exchange_name]["secretkey"],
        )
        return apikey, secretkey

    async def reload_market(self):
        self.worker_reload: asyncio.Task = asyncio.create_task(
            self.ExchangeManager.reload_markets(
                self.exchange_name, self.id, self.symbol, self.interval
            )
        )
    ...
    def on_replay_reset_worker(self, index, is_goto: bool = False):
        ...
        self.worker = FastStartThread(
            self.on_replay_mode_reset_exchange, index, is_goto
        )
        self.worker.start_thread()

    async def on_replay_mode_reset_exchange(
        self, index: int | float, is_goto: bool = False
    ):
        data = []
        if is_goto:
            _cr_time = index
        ...
        data = self.crypto_ex.fetch_ohlcv(
            self.symbol, self.interval, limit=1500, params={"until": _cr_time * 1000}
        )
    ...
    def replay_forward_update(self):
        if not self.replay_data:
            ...
            _cr_time = int(ohlcv.time)
            _ohlcv = self.crypto_ex.fetch_ohlcv(
                self.symbol, self.interval, since=_cr_time * 1000, limit=1500
            )
        ...
        pre_ohlcv = OHLCV(
            self.replay_data[self.replay_pos_i - 1][1],
            self.replay_data[self.replay_pos_i - 1][2],
            self.replay_data[self.replay_pos_i - 1][3],
            self.replay_data[self.replay_pos_i - 1][4],
            round(
                (
                    self.replay_data[self.replay_pos_i - 1][2]
                    + self.replay_data[self.replay_pos_i - 1][3]
            ...
            self.replay_data[self.replay_pos_i - 1][5],
            self.replay_data[self.replay_pos_i - 1][0] / 1000,
            0,
        )
        last_ohlcv = OHLCV(
            self.replay_data[self.replay_pos_i][1],
            ...
            self.replay_data[self.replay_pos_i][5],
            self.replay_data[self.replay_pos_i][0] / 1000,
            0,
        )
            ...
            if len(self.replay_data) > 1:
                ...
                for i in range(self.replay_pos_i, len(self.replay_data)):
                    ...
                    if self.is_running_replay:
                        ...
                        _is_add_candle = self.jp_candle.update([pre_ohlcv, last_ohlcv])
                        self.heikinashi.update(
                            self.jp_candle.candles[-2:], _is_add_candle
                        )
                        self.replay_pos_i = i
                        ...
                        if self.atkobj:
                            self.atkobj.move_entry(
                                last_point.index, last_point.high, last_point.low
                            )
        ...
        if not self.is_load_historic:
            ...
            if self.jp_candle.candles:
                ...
                if left_xrange < first_candlestick_index + 1500:
                    ...
                    self.worker_auto_load_old_data = SimpleWorker(
                        fn=self.check_signal_load_old_data
                    )
    ...
    def update_sources(
        self, source: HEIKINASHI | SMOOTH_CANDLE | JAPAN_CANDLE | N_SMOOTH_CANDLE
    ):
    ...
    def get_source(
        self, _source_name
    ) -> HEIKINASHI | SMOOTH_CANDLE | JAPAN_CANDLE | N_SMOOTH_CANDLE:
    ...
    def generate_source(self, interval):
        ...
        self.generate_source_worker(
            exchange_name=self.exchange_name, symbol=self.symbol, interval=interval
        )

    def generate_source_worker(
        self, exchange_name: str = "binanceusdm", symbol: str = "", interval: str = ""
    ):
        pass
    ...
    def set_up_exchange(self):
        ...
        self.crypto_ex, self.crypto_ex_ws = self.ExchangeManager.add_exchange(
            self.exchange_name,
            self.id,
            self.symbol,
            self.interval,
    ...
    def on_reset_exchange(self, args):
        ...
        asyncio.run(
            self.ExchangeManager.remove_exchange(
                self.exchange_name, self.id, self.symbol, self.interval
            )
        )
    ...
    async def loop_watch_ohlcv(self, symbol, interval, exchange_name):
        ...
        n = 0
        while True:
            client_socket = self.ExchangeManager.get_client_exchange(
                exchange_name, self.id, symbol, interval
            )
            ws_socket = self.ExchangeManager.get_ws_exchange(
                exchange_name, self.id, symbol, interval
            )
            ...
            if not (
                ...
                and self.exchange_name == exchange_name
            ):
                break
            if client_socket != None and ws_socket != None:
                try:
                    ...
                    if "watchOHLCV" in list(ws_socket.has.keys()):
                        if _ohlcv == []:
                            _ohlcv = client_socket.fetch_ohlcv(
                            ...
                            ohlcv = await ws_socket.watch_ohlcv(
                                symbol, interval, limit=2
                            )
                            if _ohlcv[-1][0] / 1000 == ohlcv[-1][0] / 1000:
                                _ohlcv[-1] = ohlcv[-1]
                ...
                if len(_ohlcv) >= 2 and (
                    ...
                    pre_ohlcv = OHLCV(
                        ...
                        round(
                            (_ohlcv[-2][2] + _ohlcv[-2][3] + _ohlcv[-2][4]) / 3,
                            self._precision,
                        ),
                        round(
                            (
                    ...
                    last_ohlcv = OHLCV(
                        ...
                        round(
                            (_ohlcv[-1][2] + _ohlcv[-1][3] + _ohlcv[-1][4]) / 3,
                            self._precision,
                        ),
                        round(
                            (
    ...
    def setup_indicator(self, indicator_data):
        ...
        if _group_indicator == "Basic Indicators":
            ...
            if isinstance(candle.source, JAPAN_CANDLE):
                ...
                self.auto_xrange()
                self.sig_show_process.emit(False)
            if not isinstance(candle.source, JAPAN_CANDLE) and not isinstance(
                candle.source, HEIKINASHI
            ):
    ...
    def keyPressEvent(self, ev: QKeyEvent):
        if ev.key() == Qt.Key.Key_Delete:
            ...
            if ev.keyCombination() == QKeyCombination(
                Qt.KeyboardModifier.AltModifier, Qt.Key.Key_T
            ):
                self.drawtool.draw_object_name = "draw_trenlines"
            elif ev.keyCombination() == QKeyCombination(
                Qt.KeyboardModifier.AltModifier, Qt.Key.Key_H
            ):
                self.drawtool.draw_object_name = "draw_horizontal_line"
            elif ev.keyCombination() == QKeyCombination(
                Qt.KeyboardModifier.AltModifier, Qt.Key.Key_J
            ):
                ...
                Qt.KeyboardModifier.AltModifier, Qt.Key.Key_V
            ):
