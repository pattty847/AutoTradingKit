...
from atklip.gui.components.setting_element import (
    PeriodEdit,
    MultiDevEdit,
    PriceEdit,
    ColorEdit,
...
class BasicMenu(ScrollInterface):
    ...
    def __init__(self, name: str, parent: QWidget = None, indicator=None, chart=None):
        ...
        self.setSpacing(3)
        self.sig_add_indicator.connect(
            self.add_Widget, Qt.ConnectionType.AutoConnection
        )
        self.sig_remove_indicator.connect(
            self.remove_Widget, Qt.ConnectionType.AutoConnection
        )
    ...
    def setup_setting_indicator(self):
        ...
        if _name == "input":
            ...
            for _input in list(_inputs.keys()):
                if _input == "source":
                    source = SourceEdit(
                        self, self.indicator, _input, self.chart.sources
                    )
        ...
        if _styles != {}:
            ...
            if _inputs == {}:
                ...
                qrouter.setDefaultRouteKey(
                    self.stackedWidget, self.style_widget.objectName()
                )
            else:
                ...
                self.setFixedHeight(self.stackedWidget.height() + self.pivot.height())
                qrouter.setDefaultRouteKey(
                    self.stackedWidget, self.input_widget.objectName()
                )
...
class IndicatorSettingMenu(MovingWidget):
    def __init__(self, indicator, parent: QWidget = None, chart=None):
        super(IndicatorSettingMenu, self).__init__(
            parent, f"""Setting {indicator.has["name"]}"""
        )
        self.title.btn_close.clicked.connect(
            self.deleteLater, Qt.ConnectionType.AutoConnection
        )
        "change title"
        indicator.sig_change_indicator_name.connect(
            self.change_title, Qt.ConnectionType.AutoConnection
        )
        ...
        menu.sig_change_widget.connect(
            self.update_size, Qt.ConnectionType.AutoConnection
        )
