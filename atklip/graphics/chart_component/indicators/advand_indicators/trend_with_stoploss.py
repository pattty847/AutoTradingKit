...
class TrendStopLoss(GraphicsObject):
    ...
    def __init__(self, chart) -> None:
        ...
        self.INDICATOR = TrendWithStopLoss(
    ...
    def regen_indicator(self, setdata):
        x_data, long_stoploss, short_stoploss, Uptrend, Downtrend = (
            self.INDICATOR.get_data()
        )
        setdata.emit((x_data, long_stoploss, short_stoploss, Uptrend, Downtrend))
        self.has["name"] = (
            f"""TrendSL {self.has["inputs"]["fast_period"]} {self.has["inputs"]["slow_period"]} {self.has["inputs"]["signal_period"]} {self.has["inputs"]["atr_length"]} {self.has["inputs"]["atr_multiplier"]} {self.has["inputs"]["atr_mamode"].name}"""
        )
    ...
    def load_historic_data(self, _len, setdata):
        x_data, long_stoploss, short_stoploss, Uptrend, Downtrend = (
            self.INDICATOR.get_data(stop=_len)
        )
        setdata.emit((x_data, long_stoploss, short_stoploss, Uptrend, Downtrend))

    def add_data(self, setdata):
        x_data, long_stoploss, short_stoploss, Uptrend, Downtrend = (
            self.INDICATOR.get_data(start=-3)
        )
        setdata.emit((x_data, long_stoploss, short_stoploss, Uptrend, Downtrend))
