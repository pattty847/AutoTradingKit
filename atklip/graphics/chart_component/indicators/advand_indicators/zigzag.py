...
class BasicZIGZAG(PlotDataItem):
    ...
    def get_styles(self):
        styles = {
            "pen_zz_line": self.has["styles"]["pen_zz_line"],
    ...
    def update_inputs(self, _input, _source):
        ...
        if update:
            self.has["name"] = (
                f"""ZIGZAG {self.has["inputs"]["legs"]} {self.has["inputs"]["deviation"]}"""
            )
    ...
    def update_styles(self, _input):
        ...
        if (
            _input == "pen_zz_line"
            or _input == "width_zz_line"
            or _input == "style_zz_line"
        ):
            self.setPen(
                color=self.has["styles"]["pen_zz_line"],
                width=self.has["styles"]["width_zz_line"],
                style=self.has["styles"]["style_zz_line"],
            )
    ...
    def boundingRect(self) -> QRectF:
        ...
        if self.yData is not None:
            if self.yData.size != 0:
                try:
                    h_low, h_high = np.nanmin(self.yData[_start:_stop]), np.nanmax(
                        self.yData[_start:_stop]
                    )
                except ValueError:
