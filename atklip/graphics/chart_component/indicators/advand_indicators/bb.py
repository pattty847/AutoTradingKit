...
class BasicBB(GraphicsObject):
    ...
    def __init__(self, chart) -> None:
        ...
        self.lowline = PlotDataItem(
            pen=self.has["styles"]["pen_low_line"]
        )  # for z value
    ...
    def regen_indicator(self, setdata):
        ...
        self.has["name"] = (
            f"""BB {self.has["inputs"]["length"]} {self.has["inputs"]["std_dev_mult"]} {self.has["inputs"]["type"]} {self.has["inputs"]["mamode"].name}"""
        )
    ...
    def update_styles(self, _input):
        ...
        if (
            _input == "pen_high_line"
            or _input == "width_high_line"
            or _input == "style_high_line"
        ):
            self.highline.setPen(
                color=self.has["styles"]["pen_high_line"],
                width=self.has["styles"]["width_high_line"],
                style=self.has["styles"]["style_high_line"],
            )
        elif (
            _input == "pen_center_line"
            or _input == "width_center_line"
            or _input == "style_center_line"
        ):
            self.centerline.setPen(
                color=self.has["styles"]["pen_center_line"],
                width=self.has["styles"]["width_center_line"],
                style=self.has["styles"]["style_center_line"],
            )
        elif (
            _input == "pen_low_line"
            or _input == "width_low_line"
            or _input == "style_low_line"
        ):
            self.lowline.setPen(
                color=self.has["styles"]["pen_low_line"],
                width=self.has["styles"]["width_low_line"],
                style=self.has["styles"]["style_low_line"],
            )
    ...
    def boundingRect(self) -> QRectF:
        ...
        if self.lowline.yData is not None:
            if self.lowline.yData.size != 0:
                try:
                    h_low, h_high = np.nanmin(
                        self.lowline.yData[_start:_stop]
                    ), np.nanmax(self.highline.yData[_start:_stop])
                except ValueError:
                    h_low, h_high = int(self.chart.yAxis.range[0]), int(
