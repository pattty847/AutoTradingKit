...
class EMA_SUPER_TREND_BOT(GraphicsObject):
    ...
    def __init__(self, chart) -> None:
        ...
        self.INDICATOR = N_SMOOTH_CANDLE(
            self.chart.get_precision(), self.chart.heikinashi, 3, "ema", 3
        )

        self.supertrend = SuperTrendWithStopLoss(
            self.INDICATOR, self.supertmodel.__dict__
        )

        self.cr_trend = None
        self.his_cr_trend = None

        self.ema_trend_metter = EMA_TREND_METTER(
            self.INDICATOR, self.ema_trend_metter_model.__dict__
        )
    ...
    def is_all_updated(self):
        is_updated = (
            ...
            and self.ema_trend_metter.is_current_update
            and self.supertrend.is_current_update
        )
        return is_updated
    ...
    def set_Data(self, df):
        ...
        for i in range(1, len(df)):
            ...
            ema_trend_metterdf = self.ema_trend_metter.df.loc[
                self.ema_trend_metter.df["index"] == _x - 1
            ]
            uptrend = False
            downtrend = False
            ...
            if (
                trend and SUPERTd > 0 and pre_cdl_type == "red"
            ):  #  and self.cr_trend == "up"
                ...
                trend and SUPERTd < 0 and pre_cdl_type == "green"
            ):  #   and self.cr_trend == "down"
