...
class ENTRY(GraphicsObject):
    ...
    def __init__(
        ...
        lastcandle,
        update_signal,
        _candles: JAPAN_CANDLE | HEIKINASHI | SMOOTH_CANDLE = [],
        high_color: str = "#089981",
        low_color: str = "#f23645",
    ) -> None:
        ...
        self.historic_candle = SingleHistogram(
            sig_reset_all,
            precision,
            change_lastprice,
            lastcandle,
        ...
        sig_reset_all.connect(
            self.threadpool_asyncworker, Qt.ConnectionType.AutoConnection
        )
        lastcandle.connect(
            self.threadpool_asyncworker, Qt.ConnectionType.SingleShotConnection
        )
        self.proxy_update_signal = Signal_Proxy(
            update_signal,
            slot=self.threadpool_asyncworker,
            connect_type=Qt.ConnectionType.AutoConnection,
        )

        self.threadpool_asyncworker()

    def get_height_vb(self):
        xmin, xmax = self.chart.vb.viewRange[0]
        ymin, ymax = self.chart.vb.viewRange[0]
        return ymin, ymax

    def get_min_max(self):
        x_data, y_data = self.has["inputs"]["source"].get_index_volumes(
            stop=len(self.has["inputs"]["source"].candles)
        )
    ...
    def setup_color(self, colors):
        ...
        self.has["styles"]["highcolor"] = (
            colors.get("high") if colors.get("high") else "#0ecb81"
        )
        self.has["styles"]["lowcolor"] = (
            colors.get("low") if colors.get("low") else "#f6465d"
        )
    ...
    def threadpool_asyncworker(self):
        ...
        self.worker.signals.finished.connect(
            self.sig_change_yaxis_range, Qt.ConnectionType.AutoConnection
        )
        self.worker.start()
    ...
    def draw_histogram(self, p: QPainter, value, w, x_data, index):
        if value > 0:
            self.outline_pen = mkPen(
                color=self.has["styles"]["lowcolor"], width=0.7
            )  # ,width=0.7
            p.setPen(self.outline_pen)
            p.setBrush(self.has["styles"]["low_brush"])
        else:
            self.outline_pen = mkPen(
                color=self.has["styles"]["highcolor"], width=0.7
            )  # ,width=0.7
            p.setPen(self.outline_pen)
        ...
        if value == 0:
            ...
            y_min, y_max = self.get_height_vb()
            rect = QRectF(t - w, y_min, w * 2, y_max)
            p.drawRect(rect)
    ...
    def setData(self, data) -> None:
        ...
        try:
            ...
            w = 1 / 5
        [
            self.draw_histogram(p, value, w, x_data, index)
...
class SingleHistogram(GraphicsObject):
    ...
    def __init__(
        ...
        lastcandle,
        _candles: JAPAN_CANDLE | HEIKINASHI | SMOOTH_CANDLE,
        high_color: str = "#0ecb81",
        low_color: str = "#f6465d",
    ) -> None:
        """Choose colors of candle"""
        GraphicsObject.__init__(self)
    ...
    def update_last_data(self, setdata) -> None:
        if self._canldes.first_gen:
            if self._canldes.candles != []:
                ...
                x_data, _y_data = [last_candle.index], [
                    [last_candle.open, last_candle.close, last_candle.volume]
                ]
                try:
