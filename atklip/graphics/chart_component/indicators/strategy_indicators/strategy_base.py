...
class ATKBOT(GraphicsObject):
    ...
    def __init__(self, chart) -> None:
        ...
        self.smooth_heikin = N_SMOOTH_CANDLE(
            self.chart._precision,
            self.chart.heikinashi,
            13,
        ...
        self.stoploss_smooth_heikin = N_SMOOTH_CANDLE(
            self.chart._precision,
            self.chart.heikinashi,
            3,
            self.has["inputs"]["mamode"].value,
        ...
        self.super_smoothcandle = N_SMOOTH_CANDLE(
            self.chart._precision,
            self.has["inputs"]["source"],
        ...
        self.super_trend = SuperTrend(
            self.super_smoothcandle, self.supertrend_model.__dict__
        )
    ...
    def is_all_updated(self):
        is_updated = (
            ...
            and self.super_trend.is_current_update
            and self.super_smoothcandle.is_current_update
            and self.stoploss_smooth_heikin.is_current_update
            and self.smooth_heikin.is_current_update
        )
        return is_updated
    ...
    def model(self):
        return ATKBOTModel(
            ...
            "ATKPRO",
            self.has["inputs"]["source"].source_name,
            self.has["inputs"]["key_value_long"],
            self.has["inputs"]["key_value_short"],
            self.has["inputs"]["atr_long_period"],
            self.has["inputs"]["ema_long_period"],
            self.has["inputs"]["atr_short_period"],
            self.has["inputs"]["ema_short_period"],
        )
    ...
    def update_inputs(self, _input, _source):
        ...
        if is_update:
            ...
            if (
                _input == "n_smooth_period"
                or _input == "ma_smooth_period"
                or _input == "mamode"
            ):
                self.super_smoothcandle.refresh_data(
                    self.has["inputs"]["mamode"].value,
                    self.has["inputs"]["ma_smooth_period"],
                    self.has["inputs"]["n_smooth_period"],
                )
            ...
            if (
                _input == "supertrend_length"
                or _input == "supertrend_atr_length"
                or _input == "supertrend_multiplier"
                or _input == "supertrend_atr_mamode"
            ):
                self.super_trend.change_input(
                    dict_ta_params=self.supertrend_model.__dict__
                )
    ...
    def set_Data(self, data):
        ...
        for i in range(1, len(df)):
            ...
            if (
                i - self.has["inputs"]["n_period"] - self.has["inputs"]["m_period"] - 1
                <= 0
            ):
                continue

            row = self.super_smoothcandle.df.loc[
                self.super_smoothcandle.df["index"] == _x - 1
            ]
            ...
            row_smooth_heikin = self.smooth_heikin.df.loc[
                self.smooth_heikin.df["index"] == _x - 1
            ]
            ...
            stoploss_smooth_heikin = self.stoploss_smooth_heikin.df.loc[
                self.stoploss_smooth_heikin.df["index"] == _x - 1
            ]
            stoploss_high = None
            stoploss_low = None
            ...
            super_trend_df = self.super_trend.df.loc[
                (self.super_trend.df["index"] <= _x - 1)
                & (self.super_trend.df["index"] >= _x - 4)
            ]
            ...
            if df.iloc[i - 1]["long"] == True:
                ...
                if (
                    super_trend_long_signal
                    and sm_candle_long_signal
                    and smooth_heikin_long_signal
                ):
                    ...
                    obj.setFlags(
                        obj.flags() | self.GraphicsItemFlag.ItemIgnoresTransformations
                    )
                ...
                if (
                    super_trend_short_signal
                    and sm_candle_short_signal
                    and smooth_heikin_short_signal
                ):
    ...
    def add_historic_Data(self, data):
        ...
        for i in range(1, len(df)):
            ...
            if df.iloc[i - 1]["long"] == True:
                ...
                pv_df = self.chart.jp_candle.df.loc[
                    (self.chart.jp_candle.df["index"] < _x)
                    & (
                        self.chart.jp_candle.df["index"]
                        >= _x
                        - self.has["inputs"]["n_period"]
                        - self.has["inputs"]["m_period"]
                        - 1
                    )
                ]
                pivot_point = self.check_pivot_points(
                    pv_df,
                    "low",
                    self.has["inputs"]["n_period"],
                    self.has["inputs"]["m_period"],
                ...
                if pivot_point[0]:
                    ...
                    if macd_long_signal and sm_candle_long_signal:
                        ...
                        self.list_pos[pivot_point[2]] = {
                            "stop_loss": pivot_point[1],
                            "entry_x": _x,
                            "entry_y": _val,
                            "type": "long",
                            "obj": obj,
    ...
    def add_data(self, setdata):
        xdata, _long, _short = self.INDICATOR.get_data(
            start=-int(
                self.has["inputs"]["n_period"] + self.has["inputs"]["m_period"] + 10
            )
        )
        setdata.emit((xdata, _long, _short))
