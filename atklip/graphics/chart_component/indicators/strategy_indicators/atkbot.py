...
class ATKBOT(GraphicsObject):
    ...
    def __init__(self, chart) -> None:
        ...
        self.supertrend = SuperTrendWithStopLoss(
            self.has["inputs"]["source"], self.supertmodel.__dict__
        )
        ...
        self.Source: JAPAN_CANDLE | HEIKINASHI | SMOOTH_CANDLE | N_SMOOTH_CANDLE = (
    ...
    def is_all_updated(self):
        is_updated = (
            self.supertrend.is_current_update
            and self.utbot.is_current_update
            and self.rsi.is_current_update
            and self.rsifast.is_current_update
        )
    ...
    def mafastmodel(self) -> dict:
        return MAModel(
            ...
            self.has["inputs"]["mafast_mamode"].name.lower(),
            self.has["inputs"]["mafast_type"],
            self.has["inputs"]["mafast_length"],
        )
    ...
    def maslowmodel(self) -> dict:
        return MAModel(
            ...
            "MA",
            self.chart.jp_candle.source_name,
            self.has["inputs"]["maslow_mamode"].name.lower(),
            self.has["inputs"]["maslow_type"],
            self.has["inputs"]["maslow_length"],
        )
    ...
    def connect_signals(self):
        ...
        self.Source.signal_delete.connect(
    ...
    def regen_indicator(self, setdata):
        ...
        for i in range(1, len(self.utbot.df)):
            ...
            if (
                self.utbot.df.iloc[i - 1]["long"] == True
                and SUPERTd > 0
                and rsidata > 50
            ):  #  and 90 < rsifastdata < 100: # and maslowdata < mafastdata
                ...
                self.utbot.df.iloc[i - 1]["short"] == True
                and SUPERTd < 0
                and rsidata < 50
            ):  # and SUPERTd<0 and 0 < rsifastdata < 20: # and maslowdata > mafastdata
    ...
    def update_inputs(self, _input, _source):
        ...
        if is_update:
            ...
            if (
                ...
                self.stoploss_smooth_heikin.refresh_data(
    ...
    def check_pos_is_near_pivot(self, _x: int, check_type: str):
        ...
        stoploss_smooth_heikin = self.stoploss_smooth_heikin.df.loc[
            (self.stoploss_smooth_heikin.df["index"] >= _x - 3)
            & (self.stoploss_smooth_heikin.df["index"] < _x)
        ]
    ...
    def update_Data(self, df: pd.DataFrame):
        ...
        if (
            self.utbot.df.iloc[-2]["long"] == True and SUPERTd > 0 and rsidata > 50
        ):  #  and 90 < rsifastdata < 100: # and maslowdata < mafastdata
            ...
            self.utbot.df.iloc[-2]["short"] == True and SUPERTd < 0 and rsidata < 50
        ):  # and SUPERTd<0 and 0 < rsifastdata < 20: # and maslowdata > mafastdata
