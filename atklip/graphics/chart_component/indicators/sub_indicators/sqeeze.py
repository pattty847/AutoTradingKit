...
class BaSicSqeeze(GraphicsObject):
    ...
    def regen_indicator(self, setdata):
        ...
        self.has["name"] = (
            f"""SQZ {self.has["inputs"]["mamode"].name} {self.has["inputs"]["bb_length"]} {self.has["inputs"]["bb_std"]} {self.has["inputs"]["kc_length"]} {self.has["inputs"]["kc_scalar"]}"""
        )
    ...
    def get_min_max(self):
        ...
        try:
            if len(self.histogram.y_data) > 0:
                _min, _max = np.nanmin(self.histogram.y_data), np.nanmax(
                    self.histogram.y_data
                )
                return _min, _max
