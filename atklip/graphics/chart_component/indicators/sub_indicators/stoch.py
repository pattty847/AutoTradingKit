...
from atklip.controls.models import STOCHModel
...
class BasicSTOCH(GraphicsObject):
    ...
    def __init__(self, get_last_pos_worker, chart, panel) -> None:
        ...
        self.INDICATOR = STOCH(self.has["inputs"]["source"], self.model.__dict__)
    ...
    def regen_indicator(self, setdata):
        ...
        self.has["name"] = (
            f"""STOCH {self.has["inputs"]["mamode"].name} {self.has["inputs"]["smooth_k_period"]} {self.has["inputs"]["k_period"]} {self.has["inputs"]["d_period"]}"""
        )
    ...
    def add_historic_Data(self, data):
        ...
        lb = data[1]
        cb = data[2]
        self.stoch_line.addHistoricData(xData, lb)
    ...
    def update_Data(self, data):
        ...
        lb = data[1]
        cb = data[2]
        self.stoch_line.updateData(xData, lb)
    ...
    def load_historic_data(self, _len, setdata):
        xdata, stoch, signalma = self.INDICATOR.get_data(stop=_len)
        setdata.emit((xdata, stoch, signalma))

    def update_data(self, setdata):
        xdata, stoch, signalma = self.INDICATOR.get_data(start=-1)
        setdata.emit((xdata, stoch, signalma))
