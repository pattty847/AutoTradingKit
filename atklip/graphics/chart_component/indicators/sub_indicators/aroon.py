...
class BasicAroon(GraphicsObject):
    ...
    def regen_indicator(self, setdata):
        ...
        self.has["name"] = (
            f"""Aroon {self.has["inputs"]["length"]} {self.has["inputs"]["scalar"]}"""
        )
    ...
    def get_styles(self):
        styles = {
            "pen_tsi_line": self.has["styles"]["pen_tsi_line"],
    ...
    def update_styles(self, _input):
        ...
        if (
            _input == "pen_tsi_line"
            or _input == "width_tsi_line"
            or _input == "style_tsi_line"
        ):
            self.aroon_up_line.setPen(
                color=self.has["styles"]["pen_tsi_line"],
                width=self.has["styles"]["width_tsi_line"],
                style=self.has["styles"]["style_tsi_line"],
            )
        elif (
    ...
    def boundingRect(self) -> QRectF:
        ...
        if self.aroon_down_line.yData is None:
            ...
            h_low, h_high = np.nanmin(self.aroon_down_line.yData), np.nanmax(
                self.aroon_down_line.yData
            )
        else:
            h_low, h_high = self._panel.yAxis.range[0], self._panel.yAxis.range[1]
    ...
    def get_min_max(self):
        ...
        try:
            if len(self.aroon_down_line.yData) > 0:
                _min, _max = np.nanmin(self.aroon_down_line.yData), np.nanmax(
                    self.aroon_down_line.yData
                )
