...
from atklip.controls import PD_MAType,IndicatorType,Aroon
from atklip.controls.models import AroonModel
...
class BasicAroon(GraphicsObject):
    """Aroon Indicator"""
    ...
    def __init__(self,get_last_pos_worker,chart,panel) -> None:
        ...
        self.aroon_up_line = PlotDataItem(pen="red")  # for z value
        self.aroon_up_line.setParentItem(self)
        self.aroon_down_line = PlotDataItem(pen="orange")
        self.aroon_down_line.setParentItem(self)
        ...
        self.INDICATOR  = Aroon(self.has["inputs"]["source"], self.model.__dict__)
    ...
    @property
    def model(self):
        return AroonModel(self.id,"Aroon",self.chart.jp_candle.source_name,
                        self.has["inputs"]["length"],
                        self.has["inputs"]["scalar"],)
    ...
    def regen_indicator(self,setdata):
        xdata,aroon_up,aroon_down= self.INDICATOR.get_data()  
        self.has["name"] = f"""Aroon {self.has["inputs"]["length"]} {self.has["inputs"]["scalar"]}"""
        self.sig_change_indicator_name.emit(self.has["name"])
        setdata.emit((xdata,aroon_up,aroon_down))
    ...
    def set_Data(self,data):
        ...
        lb = data[1]
        cb = data[2]
        self.aroon_up_line.setData(xData,lb)
        self.aroon_down_line.setData(xData,cb)
    ...
    def add_historic_Data(self,data):
        ...
        lb = data[1]
        cb = data[2]
        self.aroon_up_line.addHistoricData(xData,lb)
        self.aroon_down_line.addHistoricData(xData,cb)
        self.INDICATOR.is_current_update = True
    ...
    def update_Data(self,data):
        ...
        lb = data[1]
        cb = data[2]
        self.aroon_up_line.updateData(xData,lb)
        self.aroon_down_line.updateData(xData,cb)
        self.INDICATOR.is_current_update = True
    ...
    def load_historic_data(self,_len,setdata):
        xdata,aroon_up,aroon_down = self.INDICATOR.get_data(stop=_len)
        setdata.emit((xdata,aroon_up,aroon_down))
    
    def update_data(self,setdata):
        xdata,aroon_up,aroon_down = self.INDICATOR.get_data(start=-1)
        setdata.emit((xdata,aroon_up,aroon_down))
        self.last_pos.emit((self.has["inputs"]["indicator_type"],aroon_down[-1]))
        self._panel.sig_update_y_axis.emit()
        
    def get_inputs(self):
        inputs =  {"source":self.has["inputs"]["source"],
                    "length":self.has["inputs"]["length"],
                    "scalar":self.has["inputs"]["scalar"],
    ...
    def update_styles(self, _input):
        ...
        if _input == "pen_tsi_line" or _input == "width_tsi_line" or _input == "style_tsi_line":
            self.aroon_up_line.setPen(color=self.has["styles"]["pen_tsi_line"], width=self.has["styles"]["width_tsi_line"],style=self.has["styles"]["style_tsi_line"])
        elif _input == "pen_signal_line" or _input == "width_signal_line" or _input == "style_signal_line":
            self.aroon_down_line.setPen(color=self.has["styles"]["pen_signal_line"], width=self.has["styles"]["width_signal_line"],style=self.has["styles"]["style_signal_line"])
    ...
    def boundingRect(self) -> QRectF:
        return self.aroon_down_line.boundingRect()
    ...
    def boundingRect(self) -> QRectF:
        ...
        if self.aroon_down_line.yData is None:
            h_low,h_high = self._panel.yAxis.range[0],self._panel.yAxis.range[1]
        elif self.aroon_down_line.yData.size != 0:
            h_low,h_high = np.nanmin(self.aroon_down_line.yData), np.nanmax(self.aroon_down_line.yData) 
        else:
            h_low,h_high = self._panel.yAxis.range[0],self._panel.yAxis.range[1]
    ...
    def get_last_point(self):
        _time = self.aroon_down_line.xData[-1]
        _value = self.aroon_down_line.yData[-1]
        return _time,_value
    ...
    def get_min_max(self):
        ...
        _max = None
        try:
            if len(self.aroon_down_line.yData) > 0:
                _min, _max = np.nanmin(self.aroon_down_line.yData), np.nanmax(self.aroon_down_line.yData)
