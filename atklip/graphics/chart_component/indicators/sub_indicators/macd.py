...
class BasicMACD(GraphicsObject):
    ...
    def connect_signals(self):
        self.INDICATOR.sig_reset_all.connect(
            self.reset_indicator, Qt.ConnectionType.AutoConnection
        )
    ...
    def regen_indicator(self, setdata):
        ...
        self.has["name"] = (
            f"""MACD {self.has["inputs"]["mamode"].name} {self.has["inputs"]["fast_period"]} {self.has["inputs"]["slow_period"]} {self.has["inputs"]["signal_period"]} {self.has["inputs"]["type"]}"""
        )
    ...
    def get_styles(self):
        styles = {
            "pen_macd_line": self.has["styles"]["pen_macd_line"],
    ...
    def update_styles(self, _input):
        ...
        if (
            ...
            _input == "pen_high_historgram"
            or _input == "pen_low_historgram"
            or _input == "brush_high_historgram"
            or _input == "brush_low_historgram"
        ):
    ...
    def add_historic_Data(self, data):
        ...
        self.histogram.sig_load_historic_histogram.emit(
            (xData, histogram), "load_historic"
        )
    ...
    def add_worker(self):
        ...
        self.worker = FastWorker(self.update_data)
        self.worker.signals.setdata.connect(
            self.add_Data, Qt.ConnectionType.AutoConnection
        )
        self.worker.start()
