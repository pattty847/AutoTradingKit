...
from atklip.controls import PD_MAType,IndicatorType,ADX
from atklip.controls.models import ADXModel
...
class BasicADX(PlotDataItem):
    ...
    def __init__(self,get_last_pos_worker, chart,panel,clickable=True) -> None:
        ...
        self.INDICATOR  = ADX(self.has["inputs"]["source"], self.model.__dict__)
    ...
    @property
    def model(self):
        return ADXModel(self.id,"ADX",self.chart.jp_candle.source_name,
                        self.has["inputs"]["length"],self.has["inputs"]["lensig"],
                        self.has["inputs"]["adxr_length"],self.has["inputs"]["scalar"],
                        self.has["inputs"]["tvmode"],self.has["inputs"]["mamode"].name.lower())
    ...
    def regen_indicator(self,setdata):
        ...
        self.has["name"] = f"""ADX {self.has["inputs"]["length"]}"""
    ...
    def get_inputs(self):
        inputs =  {"source":self.has["inputs"]["source"],
                    ...
                    "lensig":self.has["inputs"]["lensig"],
                    "adxr_length":self.has["inputs"]["adxr_length"],
                    "scalar":self.has["inputs"]["scalar"],
                    "tvmode":self.has["inputs"]["tvmode"],
    ...
    def update_inputs(self,_input,_source):
        ...
        if update:
            self.has["name"] = f"""ADX {self.has["inputs"]["length"]} {self.has["inputs"]["mamode"].name}"""
            self.sig_change_indicator_name.emit(self.has["name"])
    ...
    def update_data(self,setdata):
        ...
        setdata.emit((xdata,y_data))
        self.last_pos.emit((IndicatorType.ADX,y_data[-1]))
