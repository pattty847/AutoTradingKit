...
class MACDHistogram(GraphicsObject):
    ...
    def __init__(self, chart, panel, has) -> None:
        ...
        self.sig_reset_histogram.connect(
            self.threadpool_asyncworker, Qt.ConnectionType.AutoConnection
        )
        self.sig_add_histogram.connect(
            self.update_asyncworker, Qt.ConnectionType.AutoConnection
        )
        self.sig_update_histogram.connect(
            self.update_asyncworker, Qt.ConnectionType.AutoConnection
        )
        self.sig_load_historic_histogram.connect(
            self.threadpool_asyncworker, Qt.ConnectionType.AutoConnection
        )
    ...
    def draw_single_volume(self, value, w, t):
        ...
        if value < 0:
            self.outline_pen = mkPen(
                color=self.has["styles"]["pen_low_historgram"], width=0.7
            )
            p.setPen(self.outline_pen)
            p.setBrush(self.has["styles"]["brush_low_historgram"])
        else:
            self.outline_pen = mkPen(
                color=self.has["styles"]["pen_high_historgram"], width=0.7
            )
            p.setPen(self.outline_pen)
    ...
    def setData(self, data) -> None:
        ...
        [
            self.draw_volume(value, w, x_data, index)
