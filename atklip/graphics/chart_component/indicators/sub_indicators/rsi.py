...
class BasicRSI(PlotDataItem):
    ...
    def regen_indicator(self, setdata):
        ...
        self.has["name"] = (
            f"""RSI {self.has["inputs"]["period"]} {self.has["inputs"]["type"]}"""
        )
    ...
    def update_inputs(self, _input, _source):
        ...
        if update:
            self.has["name"] = (
                f"""RSI {self.has["inputs"]["period"]} {self.has["inputs"]["mamode"].name}" {self.has["inputs"]["type"]}"""
            )
            self.sig_change_indicator_name.emit(self.has["name"])
