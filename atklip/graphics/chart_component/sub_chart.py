...
class SubChart(PlotWidget):
    ...
    def __init__(
        ...
        self.exchange_name, self.symbol, self.interval = (
            self.Chart.exchange_name,
            self.Chart.symbol,
            self.Chart.interval,
        )
        ...
        if self.crosshair_enabled:
            self._add_crosshair(
                kwargs.get(Crosshair.LINE_PEN, None),
        ...
        self.Chart.crosshair_x_value_change.connect(
            slot=self.xAxis.change_value, type=Qt.ConnectionType.AutoConnection
        )
    ...
    def get_exchange(self):
        return f"{self.symbol}_{self.interval}", self.exchanges.get(
    ...
    def on_reset_exchange(self, args):
        ...
        self.fast_reset_worker(
            exchange_name=exchange_name, symbol=symbol, interval=self.interval
        )
    ...
    async def reset_exchange(
        ...
        exchange = crypto_ex.setupEchange(
            apikey=self.apikey, secretkey=self.secretkey, exchange_name=exchange_name
        )
        ...
        if exchange != None:
            ...
            raise BadSymbol(
                f"{self.exchange_name} {symbol}/{interval} does not support fetch_ohlcv"
            )
        if len(data) == 0:
    ...
    async def loop_watch_ohlcv(self, crypto_ex, exchange, symbol, interval):
        ...
        while crypto_ex in list(self.exchanges.values()):
            if not (
                ...
                and exchange.id == self.exchange_name
            ):
                AppLogger.writer(
                    "INFO",
                    f"{__name__} - {exchange.id}-{symbol}-{interval} have changed to {self.exchange_name}-{self.symbol}-{self.interval}",
                )
                break
            if exchange != None:
                if (
    ...
    def auto_xrange(self):
        _min, _max = (
            self.Chart.yAxis.range[0],
            self.Chart.yAxis.range[1],
    ...
    def _add_crosshair(
        self, crosshair_pen: QtGui.QPen, crosshair_text_kwargs: dict
    ) -> None:
        self._precision = self.Chart.get_precision()
