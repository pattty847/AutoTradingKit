...
class MainWidget(QWidget, Ui_MainWidget):
    ...
    def __init__(
        ...
        self.rightbar.object.splitToolButton.clicked.connect(
            lambda: self.extend_right_menu(self.rightview)
        )
        ...
        self.drawbar.sig_delete_all.connect(
            self.chartbox_splitter.chart.delete_all_draw_obj
        )

        self.chartbox_splitter.chart.sig_reset_drawbar_favorite_btn.connect(
            self.drawbar.reset_drawbar
        )

        self.chartbox_splitter.chart.sig_reload_indicator_panel.connect(
            self.show_favorite_draw_btn
        )
        ...
        self.topbar.sig_change_symbol.connect(
            self.chartbox_splitter.chart.on_reset_exchange,
            Qt.ConnectionType.AutoConnection,
        )
        self.topbar.sig_change_inteval.connect(
            self.chartbox_splitter.chart.on_change_inteval,
            Qt.ConnectionType.AutoConnection,
        )
        self.topbar.sig_goto_date.connect(
            self.chartbox_splitter.chart.sig_goto_date, Qt.ConnectionType.AutoConnection
        )
        self.topbar.gotonow.clicked.connect(self.chartbox_splitter.chart.roll_till_now)
        self.topbar.sig_add_indicator_to_chart.connect(
            self.chartbox_splitter.create_indicator, Qt.ConnectionType.AutoConnection
        )
        ...
        self.chartbox_splitter.chart.sig_show_process.connect(
            self.progress.run_process, Qt.ConnectionType.AutoConnection
        )
        ...
        self.layout_bottom.addWidget(
            self.TabInterface
        )  # ,0,alignment=Qt.AlignmentFlag.AlignVCenter
    ...
    def draw_tool(self, tool_infor):
        current_tool, is_enabled, self.tool_name = (
            tool_infor[0],
            tool_infor[1],
            tool_infor[2],
        )  # "draw_trenlines"
