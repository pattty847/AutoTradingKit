...
class TitleBar(QWidget):
    ...
    def insertTab(
        ...
        # set the size of tab
        w = (
            self.tabBar.tabMaximumWidth()
            if self.tabBar.isScrollable()
            else self.tabBar.tabMinimumWidth()
        )
        ...
        item.closed.connect(
            lambda: self.tabBar.tabCloseRequested.emit(self.tabBar.items.index(item))
        )
