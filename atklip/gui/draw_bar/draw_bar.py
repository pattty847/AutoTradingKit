...
class DRAW_BAR(QFrame, Ui_draw_bar):
    ...
    def __init__(self, parent: QWidget = None):
        ...
        self.FIBONACCI = FIBONACCI(
            self, self.sig_draw_object_name, self.sig_add_to_favorite
        )
        ...
        self.FAVORITE = FAVORITE(
            self.parent, self.sig_add_to_favorite, self.sig_draw_object_name
        )
    ...
    def uncheck_items(self, _item: Tradingview_Button | ShowmenuButton):
        ...
        if self.current_btn == self._cursor.splitToolButton:
            self.reset_draw_bar()
            return
        if type(self.current_btn.parent()).__name__ == "FAVORITE":
            return
        if type(self.current_btn.parent()).__name__ == "Card_Item":
            return
        if self.items != []:
            for item in self.items:
                if item.splitToolButton != self.current_btn:
                    if isinstance(item.splitToolButton, Tradingview_Button):
                        ...
                        if isDarkTheme():
                            item.splitToolButton.setIcon(
                                item.splitToolButton.icon.path(Theme.DARK)
                            )
                        else:
                            item.splitToolButton.setIcon(
                                item.splitToolButton.icon.path(Theme.LIGHT)
                            )
                        ...
                        if item.splitToolButton._fluent_icon == None:
                            ...
                            item.splitToolButton.setfluentIcon(
                                item.splitToolButton._fluent_icon
                            )
            ...
            if (
                not self.current_btn.isChecked()
                and self.current_btn != self._cursor.splitToolButton
            ):
                self.current_btn = self._cursor.splitToolButton
                self.current_btn.setChecked(True)
                self.current_btn.set_icon_color()
