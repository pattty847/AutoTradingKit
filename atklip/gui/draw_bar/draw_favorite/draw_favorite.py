...
class FAVORITE(QFrame):
    ...
    def add_remove_favorite_wg(self):
        if self.splitToolButton.isChecked():
            ...
            menu = FavoriteSettingMenu(
                self.parent.chartbox_splitter.chart,
                self.sig_add_to_favorite,
                self.sig_draw_object_name,
            )
            self.parent.chartbox_splitter.chart.sig_reset_drawbar_favorite_btn.connect(
                menu.uncheck_btns
            )
            self.splitToolButton.clicked.connect(menu.deleteLater)
            x, y = (
                self.parent.chartbox_splitter.chart.x() + (_x - menu.width()),
                self.parent.chartbox_splitter.chart.y() + 2,
            )
    ...
    def set_current_tool(self, tool_infor):
        ...
        self.sig_add_to_favorite.emit(
            (self.current_tool.title, self.is_enabled, "draw_trenlines")
        )
